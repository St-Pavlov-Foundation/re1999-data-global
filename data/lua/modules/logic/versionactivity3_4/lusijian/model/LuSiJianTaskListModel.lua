-- chunkname: @modules/logic/versionactivity3_4/lusijian/model/LuSiJianTaskListModel.lua

module("modules.logic.versionactivity3_4.lusijian.model.LuSiJianTaskListModel", package.seeall)

local LuSiJianTaskListModel = class("LuSiJianTaskListModel", ListScrollModel)

function LuSiJianTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity220)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = LuSiJianConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = LuSiJianTaskMo.New()
	...
	if rewardCount > 1 then
		local allMO = LuSiJianTaskMo.New()
	...
	table.sort(dataList, LuSiJianTaskListModel.sortMO)
...
function LuSiJianTaskListModel.sortMO(objA, objB)
	local sidxA = LuSiJianTaskListModel.getSortIndex(objA)
	local sidxB = LuSiJianTaskListModel.getSortIndex(objB)
...
function LuSiJianTaskListModel.getSortIndex(objA)
...
function LuSiJianTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function LuSiJianTaskListModel:getRankDiff(mo)
...
function LuSiJianTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function LuSiJianTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, LuSiJianTaskListModel.sortMO)
...
function LuSiJianTaskListModel:getGotRewardCount(moList)
...
LuSiJianTaskListModel.instance = LuSiJianTaskListModel.New()

return LuSiJianTaskListModel
