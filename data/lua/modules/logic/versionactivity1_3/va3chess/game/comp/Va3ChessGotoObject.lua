...
slot0 = class("Va3ChessGotoObject")
...
function slot0.init(slot0)
	if slot0._target.originData.data and slot1.goToObject and Va3ChessGameController.instance.interacts:get(slot1.goToObject) then
...
function slot0.initAttract(slot0)
	...
	if slot0._target.config and slot0._target.config.interactType == Va3ChessEnum.InteractType.Item and not string.nilorempty(slot0._target.config.showParam) then
		for slot5, slot6 in pairs(string.splitToNumber(slot0._target.config.showParam, "#")) do
			logNormal("Va3ChessGotoObject initAttract : " .. tostring(slot6))
...
function slot0.updateGoToObject(slot0)
	...
	if Va3ChessGameController.instance.interacts:get(slot1) ~= nil then
...
function slot0.deleteRelation(slot0)
	if slot0._goToObjectId and Va3ChessGameController.instance.interacts:get(slot0._goToObjectId) ~= nil and slot2.goToObject ~= nil then
...
function slot0.refreshTarget(slot0)
	if slot0._target.avatar then
		...
		if slot0._goToObjectId ~= nil and slot0._goToObjectId ~= 0 then
			...
			if Va3ChessGameController.instance.interacts:get(slot0._goToObjectId) ~= nil then
				slot3 = slot4.objType
			end

			if slot3 == Va3ChessEnum.InteractType.Item or slot3 == Va3ChessEnum.InteractType.NoEffectItem then
				slot2 = true
			else
				slot1 = true
			end
...
function slot0.setMarkAttract(slot0, slot1)
	for slot5, slot6 in pairs(slot0._attractEnemyMap) do
		if Va3ChessGameController.instance.interacts:get(slot5) ~= nil then
