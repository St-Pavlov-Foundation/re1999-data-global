-- chunkname: @modules/logic/versionactivity1_3/versionactivity1_3dungeon/view/VersionActivity1_3TaskItem.lua

module("modules.logic.versionactivity1_3.versionactivity1_3dungeon.view.VersionActivity1_3TaskItem", package.seeall)

local VersionActivity1_3TaskItem = class("VersionActivity1_3TaskItem", ListScrollCell)

function VersionActivity1_3TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity1_3TaskItem:addEventListeners()
...
function VersionActivity1_3TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
end

function VersionActivity1_3TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity1_3TaskView)
		end
	end
end

function VersionActivity1_3TaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

VersionActivity1_3TaskItem.FinishKey = "VersionActivity1_3TaskItem FinishKey"

function VersionActivity1_3TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity1_3TaskItem.FinishKey)
...
function VersionActivity1_3TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity1_3TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity1_3TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity1_3Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity1_3TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity1_3TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity1_3TaskItem:refreshNormalUI()
...
function VersionActivity1_3TaskItem:refreshRewardItems()
...
function VersionActivity1_3TaskItem:refreshGetAllUI()
	return
end

function VersionActivity1_3TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity1_3TaskItem:getAnimator()
	return self.animator
end

function VersionActivity1_3TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity1_3TaskItem
