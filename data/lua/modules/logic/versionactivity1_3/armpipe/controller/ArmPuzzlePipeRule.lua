...
function slot0._cleaConnMark(slot0)
	for slot4 = 1, slot0._gameWidth do
		for slot8 = 1, slot0._gameHeight do
			slot9 = ArmPuzzlePipeModel.instance:getData(slot4, slot8)
...
function slot0._checkEntryConnect(slot0, slot1, slot2)
	...
	if slot2.pathType == ArmPuzzlePipeEnum.PathType.Order then
		slot3 = ArmPuzzlePipeModel.instance
...
function slot0._addToOpenSet(slot0, slot1, slot2, slot3, slot4)
	for slot8, slot9 in pairs(slot1.connectSet) do
		...
		if slot10 > 0 and slot10 <= slot0._gameWidth and slot11 > 0 and slot11 <= slot0._gameHeight and not slot2[ArmPuzzlePipeModel.instance:getData(slot10, slot11)] then
			slot2[slot13] = true
...
function slot0._markReachDir(slot0, slot1)
	for slot5, slot6 in pairs(slot1) do
		for slot10, slot11 in pairs(slot5.connectSet) do
			...
			if slot12 > 0 and slot12 <= slot0._gameWidth and slot13 > 0 and slot13 <= slot0._gameHeight and slot1[ArmPuzzlePipeModel.instance:getData(slot12, slot13)] then
...
function slot0._unmarkBranch(slot0)
	for slot4 = 1, slot0._gameWidth do
		for slot8 = 1, slot0._gameHeight do
			slot0:_unmarkSearchNode(ArmPuzzlePipeModel.instance:getData(slot4, slot8))
		end
	end
end
...
function slot0._unmarkSearchNode(slot0, slot1)
	...
	while slot2 ~= nil do
		if tabletool.len(slot2.entryConnect) == 1 and not slot2:isEntry() then
			...
			slot7 = ArmPuzzlePipeModel.instance:getData(slot4, slot5)
...
function slot0.setSingleConnection(slot0, slot1, slot2, slot3, slot4, slot5)
	if slot1 > 0 and slot1 <= slot0._gameWidth and slot2 > 0 and slot2 <= slot0._gameHeight then
		...
		if slot0._ruleConnect[ArmPuzzlePipeModel.instance:getData(slot1, slot2).value][slot3] and slot0._ruleConnect[slot5.value][slot4] then
...
function slot0.changeDirection(slot0, slot1, slot2)
	if slot0._ruleChange[ArmPuzzlePipeModel.instance:getData(slot1, slot2).value] then
		slot3.value = slot4
	end
...
function slot0.getRandomSkipSet(slot0)
	...
	slot3, slot4 = ArmPuzzlePipeModel.instance:getGameSize()

	for slot8, slot9 in ipairs(ArmPuzzlePipeModel.instance:getEntryList()) do
		slot10 = slot9.x
		slot11 = slot9.y
...
function slot0._insertToSet(slot0, slot1, slot2, slot3)
	if slot1 > 0 and slot1 <= slot0._gameWidth and slot2 > 0 and slot2 <= slot0._gameHeight then
		slot3[ArmPuzzlePipeModel.instance:getData(slot1, slot2)] = true
	end
end
