...
slot0 = class("JiaLaBoNaMapViewAnim", BaseView)
...
function slot0._editableInitView(slot0)
	slot0._checkTaskMO = Activity120TaskMO.New()
	...
	slot0._pathMaterialDict = {
		[JiaLaBoNaEnum.Chapter.One] = slot0._path1Mterials,
		[JiaLaBoNaEnum.Chapter.Two] = slot0._path2Mterials
	}
...
function slot0._findAnimator(slot0, slot1)
	return gohelper.findChild(slot0.viewGO, slot1):GetComponent(JiaLaBoNaEnum.ComponentType.Animator)
end

function slot0._findUIMeshMaterIals(slot0, slot1)
	slot4 = slot0:getUserDataTb_()

	RoomHelper.cArrayToLuaTable(gohelper.findChild(slot0.viewGO, slot1):GetComponentsInChildren(JiaLaBoNaEnum.ComponentType.UIMesh, true), slot4)
...
function slot0._isHasReward(slot0)
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity120) ~= nil then
		slot3 = slot0._checkTaskMO

		for slot7, slot8 in ipairs(Activity120Config.instance:getTaskByActId(Va3ChessEnum.ActivityId.Act120)) do
...
slot0.SWIRCH_SCENE_BLOCK_KEY = "JiaLaBoNaMapViewAnim_switchScene_Key"
...
function slot0._initPathAnimParams(slot0)
	slot0._pathConsDict = {}
	slot1 = Va3ChessEnum.ActivityId.Act120
	slot2 = Activity120Config.instance

	for slot6, slot7 in pairs(JiaLaBoNaEnum.ChapterPathAnimParam) do
...
function slot0._getPathPatams(slot0, slot1)
	...
	if slot0._pathConsDict[slot1] then
		slot5 = Activity120Model.instance

		for slot9, slot10 in ipairs(slot2) do
			if slot5:isEpisodeClear(slot10.episodeCfg.id) or slot10.episodeCfg.preEpisode == 0 or slot5:isEpisodeClear(slot10.episodeCfg.preEpisode) and JiaLaBoNaHelper.isOpenChapterDay(slot10.episodeCfg.chapterId) then
				slot3 = slot9
				slot4 = slot10
			end
		end
...
function slot0.playPathAnim(slot0)
	if Activity120Config.instance:getEpisodeCo(Va3ChessEnum.ActivityId.Act120, Activity120Model.instance:getCurEpisodeId()) then
...
function slot0._playPathAnim(slot0, slot1, slot2)
	...
	slot0._pathTweenId = ZProj.TweenHelper.DOTweenFloat(0, 1, JiaLaBoNaEnum.AnimatorTime.ChapterPath or 1, slot0._onPathFrame, slot0._onPathFinish, slot0, nil, EaseType.Linear)
end
