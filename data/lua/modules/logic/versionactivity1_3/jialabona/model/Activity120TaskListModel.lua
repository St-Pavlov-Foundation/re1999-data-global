...
slot0 = class("Activity120TaskListModel", ListScrollModel)
...
function slot0.init(slot0, slot1)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity120) ~= nil then
		for slot9, slot10 in ipairs(Activity120Config.instance:getTaskByActId(slot1)) do
			slot11 = Activity120TaskMO.New()
	...
	if slot4 > 1 then
		slot5 = Activity120TaskMO.New()
		slot5.id = JiaLaBoNaEnum.TaskMOAllFinishId
...
function slot0.getSortIndex(slot0)
	if slot0.id == JiaLaBoNaEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == JiaLaBoNaEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= JiaLaBoNaEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(JiaLaBoNaEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= JiaLaBoNaEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
