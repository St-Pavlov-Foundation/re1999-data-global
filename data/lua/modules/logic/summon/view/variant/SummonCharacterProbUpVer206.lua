-- chunkname: @modules/logic/summon/view/variant/SummonCharacterProbUpVer206.lua

module("modules.logic.summon.view.variant.SummonCharacterProbUpVer206", package.seeall)

local SummonCharacterProbUpVer206 = class("SummonCharacterProbUpVer206", SummonMainCharacterProbUp)

SummonCharacterProbUpVer206.preloadList = {
...
function SummonCharacterProbUpVer206:_editableInitView()
	...
	SummonCharacterProbUpVer206.super._editableInitView(self)
end

function SummonCharacterProbUpVer206:refreshSingleImage()
	self._simageline:LoadImage(ResUrl.getSummonHeroIcon("title_img_deco"))
end

function SummonCharacterProbUpVer206:unloadSingleImage()
...
function SummonCharacterProbUpVer206._parseCost(cost)
...
function SummonCharacterProbUpVer206._checkIsEnough(itemType, itemId, need)
	local has = ItemModel.instance:getItemQuantity(itemType, itemId)

	return need <= has
end

function SummonCharacterProbUpVer206:_refreshView(...)
	...
	self._currentCostInfo = SummonCharacterProbUpVer206._getCostInfo(poolId)

	SummonCharacterProbUpVer206.super._refreshView(self, ...)
end

function SummonCharacterProbUpVer206._getCostInfo(poolId)
	local res = {
		cost_num = 0,
		cost_num_before = 0,
		discountPercent01 = 0,
	...
	local cost_type, cost_id, cost_num = SummonCharacterProbUpVer206._parseCost(co.cost10)
...
function SummonCharacterProbUpVer206:_btnsummon10OnClick()
...
function SummonCharacterProbUpVer206:_refreshCost()
	local curPool = SummonMainModel.instance:getCurPool()
...
function SummonCharacterProbUpVer206:_refreshCost10()
	local curPool = SummonMainModel.instance:getCurPool()
...
return SummonCharacterProbUpVer206
