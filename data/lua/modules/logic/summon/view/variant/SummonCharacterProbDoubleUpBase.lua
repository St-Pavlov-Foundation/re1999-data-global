-- chunkname: @modules/logic/summon/view/variant/SummonCharacterProbDoubleUpBase.lua

module("modules.logic.summon.view.variant.SummonCharacterProbDoubleUpBase", package.seeall)

local SummonCharacterProbDoubleUpBase = class("SummonCharacterProbDoubleUpBase", SummonMainCharacterProbUp)

function SummonCharacterProbDoubleUpBase:_editableInitView()
	...
	self._tip = gohelper.findChild(self.viewGO, "#go_ui/current/tip/tip")
	self._freetag = gohelper.findChild(self._tip, "freetag")
	self._arrow = gohelper.findChild(self._tip, "arrow")
	self._tipHLayoutGroup = self._tip:GetComponent(gohelper.Type_HorizontalLayoutGroup)
	self._txtcurrency102.text = ""
	self._txtcurrency101.text = ""

	SummonCharacterProbDoubleUpBase.super._editableInitView(self)
end

local function _parseCost(cost)
...
local function _getCostInfo(poolId)
	local res = {
		cost_num = 0,
		cost_num_before = 0,
		discountPercent01 = 0,
	...
	local cost_type, cost_id, cost_num = _parseCost(co.cost10)
...
function SummonCharacterProbDoubleUpBase:_refreshView(...)
	...
	self._currentCostInfo = _getCostInfo(poolId)

	SummonCharacterProbDoubleUpBase.super._refreshView(self, ...)
end

function SummonCharacterProbDoubleUpBase:_btnsummon10OnClick()
...
function SummonCharacterProbDoubleUpBase:_refreshCost10()
	local curPool = SummonMainModel.instance:getCurPool()
	...
	gohelper.setActive(self._freetag, isShowDiscount)
	...
	if LangSettings.instance:isEn() then
		...
		gohelper.setActive(self._arrow, not isShowDiscount)

		local padding = self._tipHLayoutGroup.padding

		if isShowDiscount then
			self._tipHLayoutGroup.spacing = -60
			padding.left = -300
		else
			padding.left = 110
			self._tipHLayoutGroup.spacing = 20
		end
	end
end

return SummonCharacterProbDoubleUpBase
