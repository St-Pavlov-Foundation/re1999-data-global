...
slot0 = class("SummonCustomPickChoiceItem", LuaCompBase)
...
function slot0.onClickSelf(slot0)
	...
	SummonCustomPickChoiceController.instance:setSelect(slot0._mo.id)
end
...
function slot0.refreshUI(slot0)
	if not HeroConfig.instance:getHeroCO(slot0._mo.id) then
		logError("SummonCustomPickChoiceItem.onUpdateMO error, heroConfig is nil, id:" .. tostring(slot0._mo.id))
...
function slot0.refreshBaseInfo(slot0, slot1)
	if not SkinConfig.instance:getSkinCo(slot1.skinId) then
		logError("SummonCustomPickChoiceItem.onUpdateMO error, skinCfg is nil, id:" .. tostring(slot1.skinId))
...
function slot0.refreshSelect(slot0)
	gohelper.setActive(slot0._goSelected, SummonCustomPickChoiceListModel.instance:isHeroIdSelected(slot0._mo.id))
end
