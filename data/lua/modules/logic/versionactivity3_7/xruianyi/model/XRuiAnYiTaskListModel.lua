-- chunkname: @modules/logic/versionactivity3_7/xruianyi/model/XRuiAnYiTaskListModel.lua

module("modules.logic.versionactivity3_7.xruianyi.model.XRuiAnYiTaskListModel", package.seeall)

local XRuiAnYiTaskListModel = class("XRuiAnYiTaskListModel", ListScrollModel)

function XRuiAnYiTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity220)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = XRuiAnYiConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = XRuiAnYiTaskMo.New()
	...
	if rewardCount > 1 then
		local allMO = XRuiAnYiTaskMo.New()
	...
	table.sort(dataList, XRuiAnYiTaskListModel.sortMO)
...
function XRuiAnYiTaskListModel.sortMO(objA, objB)
	local sidxA = XRuiAnYiTaskListModel.getSortIndex(objA)
	local sidxB = XRuiAnYiTaskListModel.getSortIndex(objB)
...
function XRuiAnYiTaskListModel.getSortIndex(objA)
...
function XRuiAnYiTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function XRuiAnYiTaskListModel:getRankDiff(mo)
...
function XRuiAnYiTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function XRuiAnYiTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, XRuiAnYiTaskListModel.sortMO)
...
function XRuiAnYiTaskListModel:getGotRewardCount(moList)
...
XRuiAnYiTaskListModel.instance = XRuiAnYiTaskListModel.New()

return XRuiAnYiTaskListModel
