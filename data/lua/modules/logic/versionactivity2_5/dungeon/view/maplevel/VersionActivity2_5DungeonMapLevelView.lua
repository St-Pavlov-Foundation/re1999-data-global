...
function slot0._buildEpisodeName_overseas(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if LangSettings.instance:isJp() or LangSettings.instance:isEn() then
		for slot12, slot13 in ipairs(uv0) do
			if slot0.showEpisodeCo.id == slot13 then
				slot7 = true
	...
	if slot7 then
		return slot0:buildColorText(string.format("<size=90>%s</size>%s", slot1, slot2), slot3)
	else
