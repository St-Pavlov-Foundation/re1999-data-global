...
slot0 = class("VersionActivity2_1DungeonController", BaseController)
...
function slot0.openStoreView(slot0)
	if not VersionActivityEnterHelper.checkCanOpen(VersionActivity2_1Enum.ActivityId.DungeonStore) then
		return
	end
...
function slot0._internalOpenVersionActivityDungeonMapView(slot0)
	...
	if not slot0.dungeonMapViewParam.episodeId then
		slot1 = VersionActivity2_1DungeonModel.instance:getUnFinishElementEpisodeId()
		slot3 = nil

		if VersionActivity2_1DungeonModel.instance:getUnFinishStoryElements() and #slot2 > 0 then
	...
	ViewMgr.instance:openView(ViewName.VersionActivity2_1DungeonMapView, slot0.dungeonMapViewParam)
end
...
function slot1(slot0)
	...
	return string.format("2_0Dungeon_%s_%s", VersionActivity2_1Enum.ActivityId.Dungeon, slot0)
end
