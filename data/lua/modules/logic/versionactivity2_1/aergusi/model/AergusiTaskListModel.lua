...
slot0 = class("AergusiTaskListModel", ListScrollModel)
...
function slot0.refreshData(slot0)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity163) ~= nil then
		for slot8, slot9 in ipairs(AergusiConfig.instance:getTaskByActId(VersionActivity2_1Enum.ActivityId.Aergusi)) do
			slot10 = AergusiTaskMO.New()
	...
	if slot3 > 1 then
		slot4 = AergusiTaskMO.New()
		slot4.id = AergusiEnum.TaskMOAllFinishId
		slot4.activityId = VersionActivity2_1Enum.ActivityId.Aergusi
...
function slot0.getSortIndex(slot0)
	if slot0.id == AergusiEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == AergusiEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= AergusiEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(AergusiEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= AergusiEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
...
function slot0.setAniDisable(slot0, slot1)
	slot0._disableAni = slot1
end

function slot0.getAniDisableState(slot0)
	return slot0._disableAni
end
