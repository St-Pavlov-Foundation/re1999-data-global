...
slot0 = class("LanShouPaTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act164TaskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._act164TaskMO.config.jumpId)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	slot0:_onOneClickClaimReward(slot0._act164TaskMO.activityId)
...
function slot0._btngetallOnClick(slot0)
	LanShouPaController.instance:dispatchEvent(LanShouPaEvent.OneClickClaimReward, slot0._act164TaskMO.activityId)
	UIBlockHelper.instance:startBlock("LanShouPaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._act164TaskMO.config.id)
end
...
function slot0._editableInitView(slot0)
	slot0._animator = slot0.viewGO:GetComponent(LanShouPaEnum.ComponentType.Animator)
...
function slot0._editableAddEvents(slot0)
	LanShouPaController.instance:registerCallback(LanShouPaEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	LanShouPaController.instance:unregisterCallback(LanShouPaEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act164TaskMO and slot0._act164TaskMO.activityId == slot1 and slot0._act164TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	slot0._act164TaskMO = slot1
	slot0._scrollRewards.parentGameObject = slot0._view._csListScroll.gameObject

	slot0:_refreshUI()
	slot0:_moveByRankDiff(Activity164TaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._moveByRankDiff(slot0, slot1)
	if slot1 and slot1 ~= 0 then
		...
		slot0._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(slot0.viewTrs, 0, LanShouPaEnum.AnimatorTime.TaskRewardMoveUp)
	end
end
...
function slot0._refreshUI(slot0)
	if not slot0._act164TaskMO then
		return
	end

	slot2 = slot1.id ~= LanShouPaEnum.TaskMOAllFinishId
...
slot0.prefabPath = "ui/viewres/versionactivity_2_1/v2a1_tuesday/v2a1_tuesday_taskitem.prefab"
