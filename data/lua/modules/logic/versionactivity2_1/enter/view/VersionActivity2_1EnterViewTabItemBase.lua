...
local var_0_0 = class("VersionActivity2_1EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function var_0_0.afterSetData(arg_2_0)
	...
	if not arg_2_0.activityCo then
		...
		logError("VersionActivity2_1EnterViewTabItemBase.afterSetData error, no act config, actId:%s", arg_2_0.actId)
	...
	if not string.nilorempty(var_2_1) then
		UISpriteSetMgr.instance:setV2a1MainActivitySprite(arg_2_0.imageSelectTabIcon, var_2_1)
	end

	local var_2_2 = var_2_0[2]

	if not string.nilorempty(var_2_2) then
		UISpriteSetMgr.instance:setV2a1MainActivitySprite(arg_2_0.imageUnSelectTabIcon, var_2_2)
	end
...
function var_0_0.refreshNormalTag(arg_7_0)
	...
	if VersionActivity2_1EnterHelper.GetIsShowTabRemainTime(arg_7_0.actId) and var_7_0 then
		if var_7_0:getRealEndTimeStamp() - ServerTime.now() > VersionActivity2_1Enum.MaxShowTimeOffset then
			return
		end
