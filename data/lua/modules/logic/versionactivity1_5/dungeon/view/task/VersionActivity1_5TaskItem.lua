-- chunkname: @modules/logic/versionactivity1_5/dungeon/view/task/VersionActivity1_5TaskItem.lua

module("modules.logic.versionactivity1_5.dungeon.view.task.VersionActivity1_5TaskItem", package.seeall)

local VersionActivity1_5TaskItem = class("VersionActivity1_5TaskItem", ListScrollCell)

function VersionActivity1_5TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity1_5TaskItem:addEventListeners()
...
function VersionActivity1_5TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
end

function VersionActivity1_5TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity1_5TaskView)
		end
	end
end

function VersionActivity1_5TaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

VersionActivity1_5TaskItem.FinishKey = "VersionActivity1_5TaskItem FinishKey"

function VersionActivity1_5TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity1_5TaskItem.FinishKey)
...
function VersionActivity1_5TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity1_5TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity1_5TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity1_5Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity1_5TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity1_5TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity1_5TaskItem:refreshNormalUI()
...
function VersionActivity1_5TaskItem:refreshRewardItems()
...
function VersionActivity1_5TaskItem:refreshGetAllUI()
	return
end

function VersionActivity1_5TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity1_5TaskItem:getAnimator()
	return self.animator
end

function VersionActivity1_5TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity1_5TaskItem
