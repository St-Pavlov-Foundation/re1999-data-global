...
function slot0.buildEpisodeName(slot0)
	...
	if LangSettings.instance:isKr() then
		return slot0:buildColorText(string.format("<size=105>%s</size>%s", slot2, slot3), slot0.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard and "#cfccc9" or "#cfccc9")
	elseif LangSettings.instance:isEn() then
		return slot0:buildColorText(string.format("<size=100>%s</size>%s", slot2, slot3), slot5)
	else
		return slot0:buildColorText(string.format("<size=90>%s</size>%s", slot2, slot3), slot5)
	end
end
