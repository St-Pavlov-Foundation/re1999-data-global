module("modules.logic.activity.view.show.ActivityInsightShowTaskItem", package.seeall)

slot0 = class("ActivityInsightShowTaskItem", LuaCompBase)
...
function slot0._refreshBtns(slot0)
	...
	if slot0._taskMO.finishCount >= 1 then
		if not ActivityType172Model.instance:isTaskHasUsed(ActivityEnum.Activity.NewInsight, slot0._taskId) and slot0._config.itemId ~= 0 then
