-- chunkname: @modules/logic/activity/view/V2a2_SpringFestival_FullView.lua

module("modules.logic.activity.view.V2a2_SpringFestival_FullView", package.seeall)

local V2a2_SpringFestival_FullView = class("V2a2_SpringFestival_FullView", Activity101SignViewBase)

function V2a2_SpringFestival_FullView:onInitView()
...
function V2a2_SpringFestival_FullView:addEvents()
	Activity101SignViewBase.addEvents(self)
end

function V2a2_SpringFestival_FullView:removeEvents()
	Activity101SignViewBase.removeEvents(self)
end

function V2a2_SpringFestival_FullView:_editableInitView()
	self._txtLimitTime.text = ""

	self:internal_set_openMode(Activity101SignViewBase.eOpenMode.ActivityBeginnerView)
end

function V2a2_SpringFestival_FullView:onOpen()
	self:internal_onOpen()
	self:_clearTimeTick()
	TaskDispatcher.runRepeat(self._refreshTimeTick, self, 1)
end

function V2a2_SpringFestival_FullView:onClose()
	self:_clearTimeTick()
end

function V2a2_SpringFestival_FullView:onDestroyView()
	Activity101SignViewBase._internal_onDestroy(self)
	self:_clearTimeTick()
end

function V2a2_SpringFestival_FullView:_clearTimeTick()
	TaskDispatcher.cancelTask(self._refreshTimeTick, self)
end

function V2a2_SpringFestival_FullView:onRefresh()
	self:_refreshList()
	self:_refreshTimeTick()
end

function V2a2_SpringFestival_FullView:_refreshTimeTick()
