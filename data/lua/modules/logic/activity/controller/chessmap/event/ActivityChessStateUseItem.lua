...
slot0 = class("ActivityChessStateUseItem", ActivityChessStateBase)
...
function slot0.startNotifyView(slot0)
	...
	if not slot0.originData.range or not slot2 then
		logError("ActivityChessStateUseItem range = " .. tostring(slot4) .. ", interactId = " .. tostring(slot2))
...
function slot0.delayCallChoosePos(slot0)
	...
	if ActivityChessGameController.instance.interacts and slot5:get(slot0.originData.interactId) then
...
function slot0.hideAttractEffect(slot0)
	if ActivityChessGameController.instance.interacts:get(slot0.originData.interactId) and slot3.goToObject then
