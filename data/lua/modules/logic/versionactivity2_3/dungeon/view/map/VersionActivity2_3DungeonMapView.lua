...
slot0 = class("VersionActivity2_3DungeonMapView", BaseView)
...
function slot0.addEvents(slot0)
	...
	slot0:addEventCb(VersionActivity2_3DungeonController.instance, VersionActivity2_3DungeonEvent.OnHideInteractUI, slot0.showBtnUI, slot0)
...
function slot0.removeEvents(slot0)
	...
	slot0:removeEventCb(VersionActivity2_3DungeonController.instance, VersionActivity2_3DungeonEvent.OnHideInteractUI, slot0.showBtnUI, slot0)
...
function slot0.onRemoveElement(slot0, slot1)
	...
	if VersionActivity2_3DungeonModel.instance:checkStoryCanUnlock(slot1) then
...
function slot0._editableInitView(slot0)
	if CurrencyConfig.instance:getCurrencyCo(CurrencyEnum.CurrencyType.V2a3Dungeon) then
		UISpriteSetMgr.instance:setCurrencyItemSprite(slot0._imagestoreicon, string.format("%s_1", slot1 and slot1.icon))
	end

	slot0._txtstorename.text = ActivityModel.instance:getActivityInfo()[VersionActivity2_3Enum.ActivityId.DungeonStore].config.name
	...
	RedDotController.instance:addRedDot(slot0._goTaskReddot, RedDotEnum.DotNode.V2a3DungeonTask)
	slot0:_act165RedDot()
end
...
function slot0.refreshActivityCurrency(slot0)
	slot0._txtstorenum.text = GameUtil.numberDisplay(CurrencyModel.instance:getCurrency(CurrencyEnum.CurrencyType.V2a3Dungeon) and slot1.quantity or 0)
end

function slot0.onRefreshActivityState(slot0, slot1)
	if slot1 == VersionActivity2_3Enum.ActivityId.Dungeon and not (ActivityHelper.getActivityStatusAndToast(slot1) == ActivityEnum.ActivityStatus.Normal) then
		slot0:closeThis()
...
function slot0._onOpenView(slot0, slot1)
	if slot1 ~= ViewName.VersionActivity2_3DungeonMapLevelView then
		return
	end
...
function slot0.refreshRestaurantBtn(slot0)
	gohelper.setActive(slot0._btnrestaurant.gameObject, VersionActivity2_3DungeonModel.instance:isUnlockAct165Btn())
end
...
function slot0.refreshStoreRemainTime(slot0)
	if TimeUtil.OneDaySecond < ActivityModel.instance:getActMO(VersionActivity2_3Enum.ActivityId.DungeonStore):getRealEndTimeStamp() - ServerTime.now() then
