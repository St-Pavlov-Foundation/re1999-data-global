...
slot0 = class("ZhiXinQuanErTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._actTaskMO.config.listenerType ~= "EpisodeFinish" then
		if (tonumber(slot0._actTaskMO.config.jumpid) or 0) > 0 then
	...
	ViewMgr.instance:closeView(slot0._view.viewName)
end
