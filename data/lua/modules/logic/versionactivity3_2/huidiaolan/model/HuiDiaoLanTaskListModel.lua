-- chunkname: @modules/logic/versionactivity3_2/huidiaolan/model/HuiDiaoLanTaskListModel.lua

module("modules.logic.versionactivity3_2.huidiaolan.model.HuiDiaoLanTaskListModel", package.seeall)

local HuiDiaoLanTaskListModel = class("HuiDiaoLanTaskListModel", ListScrollModel)

function HuiDiaoLanTaskListModel:init(actId)
	...
	if rewardCount > 1 then
		local allMO = HuiDiaoLanTaskMo.New()
	...
	table.sort(dataList, HuiDiaoLanTaskListModel.sortMO)
...
function HuiDiaoLanTaskListModel.sortMO(objA, objB)
	local sidxA = HuiDiaoLanTaskListModel.getSortIndex(objA)
	local sidxB = HuiDiaoLanTaskListModel.getSortIndex(objB)
...
function HuiDiaoLanTaskListModel.getSortIndex(objA)
...
function HuiDiaoLanTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function HuiDiaoLanTaskListModel:getRankDiff(mo)
...
function HuiDiaoLanTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function HuiDiaoLanTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, HuiDiaoLanTaskListModel.sortMO)
...
function HuiDiaoLanTaskListModel:getGotRewardCount(moList)
...
HuiDiaoLanTaskListModel.instance = HuiDiaoLanTaskListModel.New()

return HuiDiaoLanTaskListModel
