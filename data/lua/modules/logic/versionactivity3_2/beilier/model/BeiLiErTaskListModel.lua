-- chunkname: @modules/logic/versionactivity3_2/beilier/model/BeiLiErTaskListModel.lua

module("modules.logic.versionactivity3_2.beilier.model.BeiLiErTaskListModel", package.seeall)

local BeiLiErTaskListModel = class("BeiLiErTaskListModel", ListScrollModel)

function BeiLiErTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity220)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = BeiLiErConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = BeiLiErTaskMo.New()
	...
	table.sort(dataList, BeiLiErTaskListModel.sortMO)
...
function BeiLiErTaskListModel.sortMO(objA, objB)
	local sidxA = BeiLiErTaskListModel.getSortIndex(objA)
	local sidxB = BeiLiErTaskListModel.getSortIndex(objB)
...
function BeiLiErTaskListModel.getSortIndex(objA)
...
function BeiLiErTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function BeiLiErTaskListModel:getRankDiff(mo)
...
function BeiLiErTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function BeiLiErTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, BeiLiErTaskListModel.sortMO)
...
function BeiLiErTaskListModel:getGotRewardCount(moList)
...
BeiLiErTaskListModel.instance = BeiLiErTaskListModel.New()

return BeiLiErTaskListModel
