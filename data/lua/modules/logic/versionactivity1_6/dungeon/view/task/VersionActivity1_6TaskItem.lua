-- chunkname: @modules/logic/versionactivity1_6/dungeon/view/task/VersionActivity1_6TaskItem.lua

module("modules.logic.versionactivity1_6.dungeon.view.task.VersionActivity1_6TaskItem", package.seeall)

local VersionActivity1_6TaskItem = class("VersionActivity1_6TaskItem", ListScrollCell)

function VersionActivity1_6TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity1_6TaskItem:onPlayOpenTransitionFinish()
	TaskDispatcher.cancelTask(self.onPlayOpenTransitionFinish, self)

	self._playOpenFinished = true
end

function VersionActivity1_6TaskItem:addEventListeners()
...
function VersionActivity1_6TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
end

function VersionActivity1_6TaskItem:_btnNotFinishOnClick()
	if not self._playOpenFinished then
		return
	end
...
function VersionActivity1_6TaskItem:_btnFinishAllOnClick()
	if not self._playOpenFinished then
		return
	end
...
VersionActivity1_6TaskItem.FinishKey = "VersionActivity1_6TaskItem FinishKey"

function VersionActivity1_6TaskItem:_btnFinishOnClick()
	if not self._playOpenFinished then
		return
	end

	UIBlockMgr.instance:startBlock(VersionActivity1_6TaskItem.FinishKey)
...
function VersionActivity1_6TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity1_6TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity1_6TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity1_6Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity1_6TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity1_6TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity1_6TaskItem:refreshNormalUI()
...
function VersionActivity1_6TaskItem:refreshRewardItems()
...
function VersionActivity1_6TaskItem:refreshGetAllUI()
	return
end

function VersionActivity1_6TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity1_6TaskItem:getAnimator()
	return self.animator
end

function VersionActivity1_6TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
	TaskDispatcher.cancelTask(self.onPlayOpenTransitionFinish, self)
end

return VersionActivity1_6TaskItem
