...
slot0 = class("VersionActivity1_6StoreItem", UserDataDispose)
...
function slot0.refreshGoods(slot0)
	...
	for slot5, slot6 in ipairs(slot0.groupGoodsCoList) do
		if not slot0.goodsItemList[slot5] then
			slot1 = VersionActivity1_6StoreGoodsItem.New()
...
function slot0.sortGoods(slot0, slot1)
	if (slot0.maxBuyCount ~= 0 and slot0.maxBuyCount - ActivityStoreModel.instance:getActivityGoodsBuyCount(VersionActivity1_6Enum.ActivityId.DungeonStore, slot0.id) <= 0) ~= (slot1.maxBuyCount ~= 0 and slot1.maxBuyCount - ActivityStoreModel.instance:getActivityGoodsBuyCount(VersionActivity1_6Enum.ActivityId.DungeonStore, slot1.id) <= 0) then
		if slot2 then
			return false
		end
