...
slot0 = class("ActQuNiangTaskItem", ListScrollCellExtend)
slot0.prefabPath = "ui/viewres/versionactivity_1_6/v1a6_quniang/v1a6_quniang_taskitem.prefab"
...
function slot0._btnfinishbgOnClick(slot0)
	if ActQuNiangController.instance:delayReward(ActQuNiangEnum.AnimatorTime.TaskReward, slot0._actTaskMO) then
...
function slot0._btngetallOnClick(slot0)
	if ActQuNiangController.instance:delayReward(ActQuNiangEnum.AnimatorTime.TaskReward, slot0._actTaskMO) then
		ActQuNiangController.instance:dispatchEvent(ActQuNiangEvent.OneClickClaimReward, slot0._actTaskMO.activityId)
	end
end
...
function slot0._editableAddEvents(slot0)
	slot0:addEventCb(ActQuNiangController.instance, ActQuNiangEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	slot0:removeEventCb(ActQuNiangController.instance, ActQuNiangEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._actTaskMO and slot0._actTaskMO.activityId == slot1 and slot0._actTaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	...
	slot0:_refreshUI()
	slot0:_moveByRankDiff(ActQuNiangTaskListModel.instance.instance:getRankDiff(slot1))
end
...
function slot0._moveByRankDiff(slot0, slot1)
	if slot1 and slot1 ~= 0 then
		...
		slot0._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(slot0.viewTrs, 0, ActQuNiangEnum.AnimatorTime.TaskRewardMoveUp)
	end
end
...
function slot0._refreshUI(slot0)
	...
	slot2 = slot1.id ~= ActQuNiangEnum.TaskMOAllFinishId
