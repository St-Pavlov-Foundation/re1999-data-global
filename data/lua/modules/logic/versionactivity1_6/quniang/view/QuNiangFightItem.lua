-- chunkname: @modules/logic/versionactivity1_6/quniang/view/QuNiangFightItem.lua

module("modules.logic.versionactivity1_6.quniang.view.QuNiangFightItem", package.seeall)

local QuNiangFightItem = class("QuNiangFightItem", LuaCompBase)

function QuNiangFightItem:init(go)
	self.viewGO = go
...
function QuNiangFightItem:addEventListeners()
...
function QuNiangFightItem:removeEventListeners()
	self._btnclick:RemoveClickListener()
	self._btnNormal:RemoveClickListener()
	self._btnHard:RemoveClickListener()
end

function QuNiangFightItem:onDestroy()
	return
end

function QuNiangFightItem:_btnOnClick()
	...
	ActQuNiangController.instance:dispatchEvent(ActQuNiangEvent.FightItemClick, self.index)
end

function QuNiangFightItem:_btnOnNormal()
	self:enterFight(self.config)
end

function QuNiangFightItem:_btnOnHard()
	local unLock = ActQuNiangModel.instance:isLevelUnlock(self.hardConfig.id)
...
function QuNiangFightItem:setParam(co, _index)
	self.config = co
	self.id = co.id
...
function QuNiangFightItem:_refreshUI()
	self:refreshStatus()
...
function QuNiangFightItem:refreshStatus()
	self.unlock = ActQuNiangModel.instance:isLevelUnlock(self.id)

	local hardUnlock = ActQuNiangModel.instance:isLevelUnlock(self.hardConfig.id)
...
function QuNiangFightItem:refreshStar()
	local passNormal = ActQuNiangModel.instance:isLevelPass(self.id)
	local passHard = ActQuNiangModel.instance:isLevelPass(self.hardConfig.id)
...
function QuNiangFightItem:refreshSelect(_index)
...
function QuNiangFightItem:isUnlock()
	return self.unlock
end

function QuNiangFightItem:enterFight(episodeConfig)
