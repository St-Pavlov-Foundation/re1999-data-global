...
slot0 = class("ActGeTianTaskListModel", ListScrollModel)
...
function slot0.refreshData(slot0)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.RoleActivity) ~= nil then
		for slot8, slot9 in pairs(RoleActivityConfig.instance:getActicityTaskList(ActGeTianEnum.ActivityId)) do
			slot10 = ActGeTianTaskMO.New()
	...
	if slot3 > 1 then
		slot4 = ActGeTianTaskMO.New()
		slot4.id = ActGeTianEnum.TaskMOAllFinishId
		slot4.activityId = ActGeTianEnum.ActivityId
...
function slot0.getSortIndex(slot0)
	if slot0.id == ActGeTianEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == ActGeTianEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= ActGeTianEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(ActGeTianEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= ActGeTianEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
