...
slot0 = class("GeTianFightItem", LuaCompBase)
...
function slot0._btnOnClick(slot0)
	...
	ActGeTianController.instance:dispatchEvent(ActGeTianEvent.FightItemClick, slot0.index)
end
...
function slot0._btnOnHard(slot0)
	if not ActGeTianModel.instance:isLevelUnlock(slot0.hardConfig.id) then
		GameFacade.showToast(ToastEnum.DungeonIsLockNormal)
...
function slot0.refreshStatus(slot0)
	slot0.unlock = ActGeTianModel.instance:isLevelUnlock(slot0.id)

	gohelper.setActive(slot0._goLock, not slot0.unlock)
	gohelper.setActive(slot0._goHardLock, not ActGeTianModel.instance:isLevelUnlock(slot0.hardConfig.id))
	slot0:refreshStar()
end

function slot0.refreshStar(slot0)
	slot1 = ActGeTianModel.instance:isLevelPass(slot0.id)
	slot2 = ActGeTianModel.instance:isLevelPass(slot0.hardConfig.id)
...
function slot0.enterFight(slot0, slot1)
	...
	if slot1.chapterId and slot3 and slot4 > 0 then
		ActGeTianModel.instance:setEnterFightIndex(slot0.index)
