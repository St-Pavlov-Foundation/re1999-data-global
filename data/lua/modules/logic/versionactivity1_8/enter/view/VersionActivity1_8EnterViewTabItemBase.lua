...
slot0 = class("VersionActivity1_8EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function slot0.afterSetData(slot0)
	...
	if not slot0.activityCo then
		...
		logError("VersionActivity1_8EnterViewTabItemBase.afterSetData error, no act config, actId:%s", slot0.actId)

		return
	end

	if not string.nilorempty(string.split(slot0.activityCo.tabBgPath, "#")[1]) then
		UISpriteSetMgr.instance:setV1a8MainActivitySprite(slot0.imageSelectTabIcon, slot2)
	end

	if not string.nilorempty(slot1[2]) then
		UISpriteSetMgr.instance:setV1a8MainActivitySprite(slot0.imageUnSelectTabIcon, slot3)
	end
...
function slot0.refreshNormalTag(slot0)
	...
	if VersionActivity1_8EnterHelper.GetIsShowTabRemainTime(slot0.actId) and slot1 then
		if VersionActivity1_8Enum.MaxShowTimeOffset < slot1:getRealEndTimeStamp() - ServerTime.now() then
			return
		end
