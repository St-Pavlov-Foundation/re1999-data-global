-- chunkname: @modules/logic/versionactivity1_4/act134/view/Activity134TaskItem.lua

module("modules.logic.versionactivity1_4.act134.view.Activity134TaskItem", package.seeall)

local Activity134TaskItem = class("Activity134TaskItem", ListScrollCellExtend)

function Activity134TaskItem:init(go)
	self.viewGO = go
...
function Activity134TaskItem:addEventListeners()
...
function Activity134TaskItem:_btnNotFinishOnClick()
...
function Activity134TaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

Activity134TaskItem.UI_CLICK_BLOCK_KEY = "Activity134TaskItemClick"

function Activity134TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(Activity134TaskItem.UI_CLICK_BLOCK_KEY)
...
function Activity134TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function Activity134TaskItem:secondAnimationDone()
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Activity134)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end

	UIBlockMgr.instance:endBlock(Activity134TaskItem.UI_CLICK_BLOCK_KEY)
end

function Activity134TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function Activity134TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function Activity134TaskItem:refreshNormalUI()
...
function Activity134TaskItem:refreshDesc()
	if not self.co then
		return
	end
	...
	desc = string.gsub(desc, "(GLN%-%d+)", string.format("<color=%s>%s</color>", Activity134TaskItem.ModeToColorDict.Normal, "%1"))

	if mode then
		desc = string.gsub(desc, "(%[.-%])", string.format("<color=%s>%s</color>", Activity134TaskItem.ModeToColorDict[mode], "%1"))
	end

	self.txttaskdesc.text = desc
end

function Activity134TaskItem:refreshRewardItems()
...
function Activity134TaskItem:refreshGetAllUI()
	return
end

function Activity134TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxProgress and self.taskMo.hasFinished
end

function Activity134TaskItem:getAnimator()
	return self.animator
end

function Activity134TaskItem:onDestroyView()
	return
end

return Activity134TaskItem
