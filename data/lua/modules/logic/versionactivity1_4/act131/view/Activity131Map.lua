...
slot0 = class("Activity131Map", BaseView)
...
function slot0._initMap(slot0)
	slot0._sceneRoot = UnityEngine.GameObject.New("Activity131Map")
	...
	slot7 = "ui/viewres/versionactivity_1_4/v1a4_6role/v1a4_role6_mapinteractiveitem.prefab"
	slot0._mapLoader = MultiAbLoader.New()

	for slot11, slot12 in ipairs(string.split(Activity131Model.instance:getCurMapConfig().scenes, "#")) do
	...
	slot0._mapLoader:startLoad(function (slot0)
		...
		slot2 = Activity131Model.instance:getEpisodeCurSceneIndex(Activity131Model.instance:getCurEpisodeId())
...
function slot0._checkInitElements(slot0)
	...
	slot2 = VersionActivity1_4Enum.ActivityId.Role6
	slot3 = Activity131Model.instance:getCurEpisodeId()

	if Activity131Model.instance:getCurMapInfo().progress == Activity131Enum.ProgressType.AfterStory then
		Activity131Controller.instance:dispatchEvent(Activity131Event.ShowFinish)

		return
	end

	if Activity131Config.instance:getActivity131EpisodeCo(slot2, slot3).elements ~= "" then
		for slot9, slot10 in ipairs(string.splitToNumber(slot4.elements, "#")) do
			for slot14, slot15 in pairs(slot1.act131Elements) do
	...
	for slot8, slot9 in pairs(slot1.act131Elements) do
		if not slot9.isFinish then
			if slot9.index <= #slot9.typeList and slot9.index ~= 0 then
...
function slot0._onGamepadKeyDown(slot0, slot1)
	if slot1 == GamepadEnum.KeyCode.A then
		...
		for slot8 = 0, UnityEngine.Physics2D.RaycastAll(slot2.origin, slot2.direction).Length - 1 do
			if MonoHelper.getLuaComFromGo(slot3[slot8].transform.parent.gameObject, Activity131MapElement) then
				slot10:onDown()
...
function slot0.onOpen(slot0)
	...
	MainCameraMgr.instance:addView(ViewName.Activity131GameView, slot0._initCamera, nil, slot0)
end
...
function slot0._showElements(slot0)
	...
	for slot8, slot9 in ipairs(Activity131Model.instance:getEpisodeElements(Activity131Model.instance:getCurEpisodeId())) do
		if not string.nilorempty(Activity131Config.instance:getActivity131ElementCo(VersionActivity1_4Enum.ActivityId.Role6, slot9.elementId).res) then
	...
	for slot9 = #slot2, 1, -1 do
		if not slot10:isAvailable() then
			slot0:_removeElement(slot10, (slot4[Activity131Config.instance:getActivity131ElementCo(slot3, slot2[slot9].elementId).res] and #slot12 or 0) <= 1)
		else
	...
	for slot9 = #slot5, 1, -1 do
		slot0:_addElement(slot10, (slot4[Activity131Config.instance:getActivity131ElementCo(slot3, slot5[slot9].elementId).res] and #slot12 or 0) <= 1)
	end
end
...
function slot0._addElement(slot0, slot1, slot2)
	...
	if not gohelper.findChild(slot0._elementRoots[Activity131Model.instance:getEpisodeCurSceneIndex(Activity131Model.instance:getCurEpisodeId())], tostring(slot1.elementId)) then
		gohelper.addChild(slot0._elementRoots[slot5], UnityEngine.GameObject.New(tostring(slot1.elementId)))
	else
		MonoHelper.removeLuaComFromGo(slot6, Activity131MapElement)
		gohelper.destroyAllChildren(slot6)
	end

	slot3 = MonoHelper.addLuaComOnceToGo(slot6, Activity131MapElement, {
		slot1,
		slot0
	})
...
function slot0._clickElement(slot0, slot1, slot2)
	...
	if slot3:getType() == Activity131Enum.ElementType.Battle then
		Activity131Controller.instance:dispatchEvent(Activity131Event.TriggerBattleElement, slot3)
	elseif slot5 == Activity131Enum.ElementType.General then
		AudioMgr.instance:trigger(tonumber(slot3.config.param))
		Activity131Rpc.instance:sendAct131GeneralRequest(VersionActivity1_4Enum.ActivityId.Role6, Activity131Model.instance:getCurEpisodeId(), slot4)
	elseif slot5 == Activity131Enum.ElementType.Respawn then
		-- Nothing
	elseif slot5 == Activity131Enum.ElementType.Dialog then
		AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_warnopen)
		Activity131Controller.instance:openActivity131DialogView(slot3)
	elseif slot5 == Activity131Enum.ElementType.TaskTip then
		Activity131Controller.instance:dispatchEvent(Activity131Event.RefreshTaskTip, slot3)
	elseif slot5 == Activity131Enum.ElementType.SetValue then
		Activity131Controller.instance:dispatchEvent(Activity131Event.UnlockCollect, slot3)
	elseif slot5 == Activity131Enum.ElementType.ChangeScene then
		slot0:_changeScene(tonumber(string.split(slot3.config.param, "#")[slot3.index + 1]), slot3)
	elseif slot5 == Activity131Enum.ElementType.LogStart or slot5 == Activity131Enum.ElementType.LogEnd then
		Activity131Controller.instance:dispatchEvent(Activity131Event.TriggerLogElement, slot3)
	end
end
...
function slot0._changeScene(slot0, slot1, slot2)
	...
	if slot2 and slot2.elementId then
		Activity131Rpc.instance:sendAct131GeneralRequest(VersionActivity1_4Enum.ActivityId.Role6, Activity131Model.instance:getCurEpisodeId(), slot2.elementId, slot0._onGeneralSuccess, slot0)
	end
end
...
function slot0._onGeneralSuccess(slot0)
	slot0:_showElements()
end
...
function slot0._addEvents(slot0)
	...
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnGeneralGameSuccess, slot0._showElements, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnElementUpdate, slot0._showElements, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnRestartEpisodeSuccess, slot0._onRestartSet, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnDialogMarkSuccess, slot0._showElements, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnClickElement, slot0._OnClickElement, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.AutoStartElement, slot0._checkInitElements, slot0)
	slot0:addEventCb(Activity131Controller.instance, Activity131Event.OnDialogMarkSuccess, slot0._checkInitElements, slot0)
end
...
function slot0._removeEvents(slot0)
	...
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnGeneralGameSuccess, slot0._showElements, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnElementUpdate, slot0._showElements, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnRestartEpisodeSuccess, slot0._onRestartSet, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnDialogMarkSuccess, slot0._showElements, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnClickElement, slot0._OnClickElement, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.AutoStartElement, slot0._checkInitElements, slot0)
	slot0:removeEventCb(Activity131Controller.instance, Activity131Event.OnDialogMarkSuccess, slot0._checkInitElements, slot0)
end
