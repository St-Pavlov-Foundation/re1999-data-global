...
slot0 = class("Activity130TaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act130TaskMO and slot0._act130TaskMO.config.jumpId and slot1 > 0 then
		GameFacade.jump(slot1)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	if Activity130Controller.instance:delayReward(Activity130Enum.AnimatorTime.TaskReward, slot0._act130TaskMO) then
		slot0:_onOneClickClaimReward(slot0._act130TaskMO.activityId)
	end
end

function slot0._btngetallOnClick(slot0)
	if Activity130Controller.instance:delayReward(Activity130Enum.AnimatorTime.TaskReward, slot0._act130TaskMO) then
		Activity130Controller.instance:dispatchEvent(Activity130Event.OneClickClaimReward, slot0._act130TaskMO.activityId)
	end
end
...
function slot0._editableAddEvents(slot0)
	Activity130Controller.instance:registerCallback(Activity130Event.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	Activity130Controller.instance:unregisterCallback(Activity130Event.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act130TaskMO and slot0._act130TaskMO.activityId == slot1 and slot0._act130TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	...
	slot0._act130TaskMO = slot1

	slot0:_refreshUI()
	slot0:_moveByRankDiff(Activity130TaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._moveByRankDiff(slot0, slot1)
	if slot1 and slot1 ~= 0 then
		...
		slot0._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(slot0.viewTrs, 0, Activity130Enum.AnimatorTime.TaskRewardMoveUp)
	end
end
...
function slot0._refreshUI(slot0)
	if not slot0._act130TaskMO then
		return
	end

	slot2 = slot1.id ~= Activity130Enum.TaskMOAllFinishId
...
slot0.prefabPath = "ui/viewres/versionactivity_1_4/v1a4_37role/v1a4_role37_taskitem.prefab"
