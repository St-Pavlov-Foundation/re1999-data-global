...
slot0 = class("Activity130TaskListModel", ListScrollModel)
...
function slot0.refreshData(slot0)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity130) ~= nil then
		for slot8, slot9 in ipairs(Activity130Config.instance:getTaskByActId(Activity130Enum.ActivityId.Act130)) do
			slot10 = Activity130TaskMO.New()
	...
	if slot3 > 1 then
		slot4 = Activity130TaskMO.New()
		slot4.id = Activity130Enum.TaskMOAllFinishId
		slot4.activityId = Activity130Enum.ActivityId.Act130
...
function slot0.getSortIndex(slot0)
	if slot0.id == Activity130Enum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == Activity130Enum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= Activity130Enum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(Activity130Enum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= Activity130Enum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
