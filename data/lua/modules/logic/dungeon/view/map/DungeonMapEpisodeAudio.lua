...
slot0 = class("DungeonMapEpisodeAudio", LuaCompBase)
...
function slot0.onUpdate(slot0)
	if slot0.scroll and not slot0._isDisposed then
		slot1 = nil
		slot1 = slot0.scroll.content and recthelper.getAnchorX(slot0.scroll.content) * slot0._pixelOffsetMoveFactor or slot0.scroll.horizontalNormalizedPosition * slot0._normalizeOffsetMoveFactor
...
function slot0._onValueChangeSampleOffset(slot0, slot1)
	if slot0._scrollAudioTime and slot0._scrollSampleOffsetX then
		if slot0._intervalSampleOffset <= Time.realtimeSinceStartup - slot0._scrollAudioTime then
			...
			slot0._scrollSampleOffsetX = slot1
		end
	else
		slot0._scrollAudioTime = Time.realtimeSinceStartup
		slot0._scrollSampleOffsetX = slot1
...
function slot0._onUpdateRTPCSpeed(slot0, slot1)
	slot2 = math.abs(slot1 - slot0._scrollSampleOffsetX)
...
function slot0._onValueChangeCheckReplay(slot0, slot1)
	if slot0._isScrollPlaying and slot0._scrollPlayingStartTime and slot0._scrollSampleOffsetX and slot0._scrollSfxDuration <= Time.realtimeSinceStartup - slot0._scrollPlayingStartTime then
		slot0:_resetPlayingStatus(slot1)
	end
end
