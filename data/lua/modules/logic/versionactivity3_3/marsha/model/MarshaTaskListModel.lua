-- chunkname: @modules/logic/versionactivity3_3/marsha/model/MarshaTaskListModel.lua

module("modules.logic.versionactivity3_3.marsha.model.MarshaTaskListModel", package.seeall)

local MarshaTaskListModel = class("MarshaTaskListModel", ListScrollModel)

function MarshaTaskListModel:init(actId)
	...
	table.sort(dataList, MarshaTaskListModel.sortMO)
...
function MarshaTaskListModel.sortMO(objA, objB)
	local sidxA = MarshaTaskListModel.getSortIndex(objA)
	local sidxB = MarshaTaskListModel.getSortIndex(objB)
...
function MarshaTaskListModel.getSortIndex(objA)
...
function MarshaTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function MarshaTaskListModel:getRankDiff(mo)
...
function MarshaTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function MarshaTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, MarshaTaskListModel.sortMO)
...
function MarshaTaskListModel:getGotRewardCount(moList)
...
MarshaTaskListModel.instance = MarshaTaskListModel.New()

return MarshaTaskListModel
