...
slot0 = class("Season1_4MarketView", BaseView)
...
function slot0._setLevels(slot0)
	...
	if slot0._layer <= 4 then
		for slot7 = 1, 6 do
			slot0._showLvItems[slot7] = Season1_4MarketShowLevelItem.New()
...
function slot0.refreshEquipCardItem(slot0)
	...
	if slot1.firstPassEquipId > 0 then
		if not slot0._rewardItems[1].itemIcon then
			slot0._rewardItems[1].itemIcon = Season1_4CelebrityCardItem.New()
