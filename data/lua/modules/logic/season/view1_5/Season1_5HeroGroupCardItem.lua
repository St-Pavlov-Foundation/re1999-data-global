...
slot0 = class("Season1_5HeroGroupCardItem", UserDataDispose)
...
function slot0.setData(slot0, slot1, slot2, slot3, slot4)
	...
	slot0._equipUid = slot3
	slot0._layer = Activity104Model.instance:getAct104CurLayer()
	slot0.id = slot1.id
	slot0._hasUseSeasonEquipCard = false
	slot0.hasTrialEquip = slot4
...
function slot0.updateView(slot0)
	...
	if slot0.hasTrialEquip and slot0:getEquipId(slot1, slot2) == 0 then
	...
	if slot0.posUnlock then
		if slot0:getEquipId(slot1, slot2) ~= 0 then
			if not slot0._seasonCardItem then
				slot0._seasonCardItem = Season1_5CelebrityCardItem.New()
...
function slot0.canDrag(slot0)
	...
	slot1, slot2 = slot0:getEquipId()

	if not slot1 or slot1 == 0 or not slot2 then
		return false
	end
