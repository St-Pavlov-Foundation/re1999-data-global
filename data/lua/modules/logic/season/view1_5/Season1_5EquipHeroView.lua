...
slot0 = class("Season1_5EquipHeroView", BaseView)
...
function slot0.refreshSlot(slot0, slot1)
	...
	if Activity104EquipItemListModel.instance.curEquipMap[slot1] == Activity104EquipItemListModel.EmptyUid then
		...
		elseif Activity104EquipItemListModel.isTrialEquip(slot3) then
			slot4:updateData(string.splitToNumber(slot3, "#")[1])
		end
	end
end
...
function slot0.refreshDesc(slot0, slot1, slot2)
	if slot2 == Activity104EquipItemListModel.EmptyUid then
		...
		if not (Activity104EquipItemListModel.instance:getEquipMO(slot2) and slot3.itemId) and Activity104EquipItemListModel.isTrialEquip(slot2) then
			slot4 = string.splitToNumber(slot2, "#")[1]
		end

		if slot4 then
			if SeasonConfig.instance:getSeasonEquipCo(slot4) then
				slot1.txtName.text = string.format("[%s]", slot5.name)
				slot6 = SeasonEquipMetaUtils.getCareerColorDarkBg(slot4)

				slot0:refreshProps(slot5, slot1, slot6)
				slot0:refreshSkills(slot5, slot1, slot6)
			else
				logError(string.format("can't find season equip config, id = [%s]", slot4))
			end
		else
...
function slot0.getOrCreateSlotIcon(slot0, slot1)
	if not slot1.icon then
		slot1.icon = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], slot1.goPos, "icon"), Season1_5CelebrityCardEquip)
	end
