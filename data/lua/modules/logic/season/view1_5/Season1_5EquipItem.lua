-- chunkname: @modules/logic/season/view1_5/Season1_5EquipItem.lua

module("modules.logic.season.view1_5.Season1_5EquipItem", package.seeall)

local Season1_5EquipItem = class("Season1_5EquipItem", ListScrollCellExtend)

function Season1_5EquipItem:init(go)
	Season1_5EquipItem.super.init(self, go)
...
function Season1_5EquipItem:addEvents()
	return
end

function Season1_5EquipItem:removeEvents()
	return
end

function Season1_5EquipItem:onUpdateMO(mo)
	self._mo = mo
	self._uid = mo.id
...
Season1_5EquipItem.ColumnCount = 6
Season1_5EquipItem.AnimRowCount = 4
Season1_5EquipItem.OpenAnimTime = 0.06
Season1_5EquipItem.OpenAnimStartTime = 0.05

function Season1_5EquipItem:checkPlayAnim()
...
function Season1_5EquipItem:onDelayPlayOpen()
	TaskDispatcher.cancelTask(self.onDelayPlayOpen, self)
	self._animator:Play("open", 0, 0)
...
function Season1_5EquipItem:refreshUI()
...
function Season1_5EquipItem:refreshDeckCount()
	local itemUid = self._uid
...
function Season1_5EquipItem:refreshEquipedHero(heroUid)
...
function Season1_5EquipItem:refreshIcon(itemId, itemUid)
...
function Season1_5EquipItem:checkCreateIcon()
...
function Season1_5EquipItem:setRoleIconDark(value)
	if value then
...
function Season1_5EquipItem:onClickSelf()
	local itemUid = self._uid
...
function Season1_5EquipItem:checkClickNew(itemUid)
...
Season1_5EquipItem.Toast_Same_Card = 2851
Season1_5EquipItem.Toast_Wrong_Type = 2852
Season1_5EquipItem.Toast_MainRole_Wrong_Type = 2854
Season1_5EquipItem.Toast_Other_Hero_Equiped = 2853
Season1_5EquipItem.Toast_Career_Wrong = 2859
Season1_5EquipItem.Toast_Slot_Lock = 67

function Season1_5EquipItem:checkCanNotEquipWithToast(itemUid)
	if Activity104EquipItemListModel.instance:slotIsLock(Activity104EquipItemListModel.instance.curSelectSlot) then
		GameFacade.showToast(Season1_5EquipItem.Toast_Slot_Lock)
	...
	if itemMO then
		...
		if isTypeWrong then
			if Activity104EquipItemListModel.instance.curPos == Activity104EquipItemListModel.MainCharPos then
				GameFacade.showToast(Season1_5EquipItem.Toast_MainRole_Wrong_Type)
			else
				GameFacade.showToast(Season1_5EquipItem.Toast_Wrong_Type)
			end
		...
		if isCareerWrong then
			GameFacade.showToast(Season1_5EquipItem.Toast_Career_Wrong)
...
function Season1_5EquipItem:onDestroyView()
	if self.icon then
