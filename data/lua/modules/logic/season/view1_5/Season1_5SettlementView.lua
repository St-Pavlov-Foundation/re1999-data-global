...
slot0 = class("Season1_5SettlementView", BaseViewExtended)
...
function slot0._showGetCardView(slot0)
	if slot0.equip_cards then
		...
		slot0._showTipsFlow:addWork(Season1_5SettlementTipsWork.New())
...
function slot0._onRewardItemShow(slot0, slot1, slot2, slot3)
	if slot2.materilType == MaterialEnum.MaterialType.EquipCard then
		Season1_5CelebrityCardItem.New():init(gohelper.findChild(gohelper.findChild(slot1, "go_prop"), "cardicon"), slot2.materilId)
...
function slot0._onHeroItemLoaded(slot0)
	...
	if not FightModel.instance.curFightModel then
		...
		for slot15 = 1, 4 do
			if slot8[slot15] then
				if lua_hero_trial.configDict[slot8[slot15].trialId][0] then
					table.insert(slot0._heroList, slot0:openSubView(Season1_5SettlementHeroItem, slot0._hero_obj_list[slot15], nil, slot7, nil, slot8[slot15].equipUid, slot11[tostring(slot16.heroId - 1099511627776.0)] and slot11[slot17].equipUid, nil, slot8[slot15]))
				end
			elseif slot16 ~= "0" then
				table.insert(slot0._heroList, slot0:openSubView(Season1_5SettlementHeroItem, slot0._hero_obj_list[slot15], nil, slot7, slot16, slot9[slot15] ~= "0" and slot10[slot16] and slot10[slot16].equipUid, slot16 ~= "0" and slot11[slot16] and slot11[slot16].equipUid))
			end
		end
		...
		if slot11["-100000"] and slot11[slot12].equipUid and slot11[slot12].equipUid[1] and slot13 ~= "0" then
			...
			slot0:openSubView(Season1_5CelebrityCardGetItem, Season1_5CelebrityCardItem.AssetPath, slot0._gosupercard, slot13, true)
		end
	else
...
function slot0._onGetFightRecordGroupReply(slot0, slot1)
	for slot5 = 1, 4 do
		...
		if slot7 ~= "0" then
			table.insert(slot0._heroList, slot0:openSubView(Season1_5SettlementHeroItem, slot6, nil, true, slot7, slot8, slot9, slot10))
		end
	end
	...
	if slot1.replay_activity104Equip_data["-100000"] then
		...
		slot0:openSubView(Season1_5CelebrityCardGetItem, Season1_5CelebrityCardItem.AssetPath, slot0._gosupercard, nil, , slot3)
	end
end
...
function slot0._onAwardItemShow(slot0, slot1, slot2, slot3)
	...
	if slot2.materilType == MaterialEnum.MaterialType.EquipCard then
		Season1_5CelebrityCardItem.New():init(gohelper.findChild(slot1, "cardicon"), slot2.materilId)
