-- chunkname: @modules/logic/season/view1_3/Season1_3EquipItem.lua

module("modules.logic.season.view1_3.Season1_3EquipItem", package.seeall)

local Season1_3EquipItem = class("Season1_3EquipItem", ListScrollCellExtend)

function Season1_3EquipItem:init(go)
	Season1_3EquipItem.super.init(self, go)
...
function Season1_3EquipItem:addEvents()
	return
end

function Season1_3EquipItem:removeEvents()
	return
end

function Season1_3EquipItem:onUpdateMO(mo)
	self._mo = mo
...
Season1_3EquipItem.ColumnCount = 6
Season1_3EquipItem.AnimRowCount = 4
Season1_3EquipItem.OpenAnimTime = 0.06
Season1_3EquipItem.OpenAnimStartTime = 0.05

function Season1_3EquipItem:checkPlayAnim()
...
function Season1_3EquipItem:onDelayPlayOpen()
	TaskDispatcher.cancelTask(self.onDelayPlayOpen, self)
	self._animator:Play("open", 0, 0)
...
function Season1_3EquipItem:refreshUI()
...
function Season1_3EquipItem:refreshDeckCount()
	local itemUid = self._itemMO.uid
...
function Season1_3EquipItem:refreshEquipedHero(heroUid)
...
function Season1_3EquipItem:refreshIcon(itemUid)
...
function Season1_3EquipItem:checkCreateIcon()
...
function Season1_3EquipItem:setRoleIconDark(value)
	if value then
...
function Season1_3EquipItem:onClickSelf()
	local itemUid = self._itemMO.uid
...
function Season1_3EquipItem:checkClickNew(itemUid)
...
Season1_3EquipItem.Toast_Same_Card = 2851
Season1_3EquipItem.Toast_Wrong_Type = 2852
Season1_3EquipItem.Toast_MainRole_Wrong_Type = 2854
Season1_3EquipItem.Toast_Other_Hero_Equiped = 2853
Season1_3EquipItem.Toast_Career_Wrong = 2859
Season1_3EquipItem.Toast_Slot_Lock = 67

function Season1_3EquipItem:checkCanNotEquipWithToast(itemUid)
	if Activity104EquipItemListModel.instance:slotIsLock(Activity104EquipItemListModel.instance.curSelectSlot) then
		GameFacade.showToast(Season1_3EquipItem.Toast_Slot_Lock)
	...
	if itemMO then
		...
		if isTypeWrong then
			if Activity104EquipItemListModel.instance.curPos == Activity104EquipItemListModel.MainCharPos then
				GameFacade.showToast(Season1_3EquipItem.Toast_MainRole_Wrong_Type)
			else
				GameFacade.showToast(Season1_3EquipItem.Toast_Wrong_Type)
			end
		...
		if isCareerWrong then
			GameFacade.showToast(Season1_3EquipItem.Toast_Career_Wrong)
...
function Season1_3EquipItem:onDestroyView()
	if self.icon then
