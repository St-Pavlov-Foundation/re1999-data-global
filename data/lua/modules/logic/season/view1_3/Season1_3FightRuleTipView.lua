...
slot0 = class("Season1_3FightRuleTipView", BaseView)
...
function slot0._editableInitView(slot0)
	slot0.labelTab = slot0:createTab(slot0._goLabel, Activity104Enum.RuleTab.Rule)
	slot0.cardTab = slot0:createTab(slot0._goCard, Activity104Enum.RuleTab.Card)
...
function slot0.onOpen(slot0)
	slot0._ruleList = Season1_3FightRuleView.getRuleList()
	slot0._cardList = Activity104Model.instance:getFightCardDataList()

	if #slot0._ruleList > 0 then
		slot0:switchTab(Activity104Enum.RuleTab.Rule)
	else
		slot0:switchTab(Activity104Enum.RuleTab.Card)
	end
end
...
function slot0.getTabActive(slot0, slot1)
	if slot1 == Activity104Enum.RuleTab.Card then
