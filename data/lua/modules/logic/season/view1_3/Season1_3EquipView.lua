...
slot0 = class("Season1_3EquipView", BaseView)
...
function slot0.refreshDesc(slot0, slot1, slot2, slot3)
	...
	if slot2 == Activity104EquipItemListModel.EmptyUid then
		...
		if Activity104EquipItemListModel.instance:getEquipMO(slot2) then
			if SeasonConfig.instance:getSeasonEquipCo(slot4.itemId) then
				slot1.txtName.text = string.format("[%s]", slot5.name)
				slot6 = nil

				if Activity104EquipItemListModel.instance.curSelectSlot ~= slot3 then
					slot7 = SeasonEquipMetaUtils.getCareerColorDarkBg(slot4.itemId) .. SeasonEquipMetaUtils.No_Effect_Alpha
					slot6 = "#cac8c5" .. SeasonEquipMetaUtils.No_Effect_Alpha
				else
					slot6 = "#ec7731"
				end

				SLFramework.UGUI.GuiHelper.SetColor(slot1.txtName, slot6)
				slot0:refreshProps(slot5, slot1, slot7)
				slot0:refreshSkills(slot5, slot1, slot7)
			else
				logError(string.format("can't find season equip config, id = [%s]", slot4.itemId))
			end
		else
...
function slot0.getOrCreateSlotIcon(slot0, slot1)
	if not slot1.icon then
		slot1.icon = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], slot1.goPos, "icon"), Season1_3CelebrityCardEquip)
	end
