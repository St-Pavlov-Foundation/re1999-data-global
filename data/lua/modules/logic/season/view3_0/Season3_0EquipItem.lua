-- chunkname: @modules/logic/season/view3_0/Season3_0EquipItem.lua

module("modules.logic.season.view3_0.Season3_0EquipItem", package.seeall)

local Season3_0EquipItem = class("Season3_0EquipItem", ListScrollCellExtend)

function Season3_0EquipItem:init(go)
	Season3_0EquipItem.super.init(self, go)
...
function Season3_0EquipItem:addEvents()
	return
end

function Season3_0EquipItem:removeEvents()
	return
end

function Season3_0EquipItem:onUpdateMO(mo)
	self._mo = mo
	self._uid = mo.id
...
Season3_0EquipItem.ColumnCount = 6
Season3_0EquipItem.AnimRowCount = 4
Season3_0EquipItem.OpenAnimTime = 0.06
Season3_0EquipItem.OpenAnimStartTime = 0.05

function Season3_0EquipItem:checkPlayAnim()
...
function Season3_0EquipItem:onDelayPlayOpen()
	TaskDispatcher.cancelTask(self.onDelayPlayOpen, self)
	self._animator:Play("open", 0, 0)
...
function Season3_0EquipItem:refreshUI()
...
function Season3_0EquipItem:refreshDeckCount()
	local itemUid = self._uid
...
function Season3_0EquipItem:refreshEquipedHero(heroUid)
...
function Season3_0EquipItem:refreshIcon(itemId, itemUid)
...
function Season3_0EquipItem:checkCreateIcon()
...
function Season3_0EquipItem:setRoleIconDark(value)
	if value then
...
function Season3_0EquipItem:onClickSelf()
	local itemUid = self._uid
...
function Season3_0EquipItem:checkClickNew(itemUid)
...
Season3_0EquipItem.Toast_Same_Card = 2851
Season3_0EquipItem.Toast_Wrong_Type = 2852
Season3_0EquipItem.Toast_MainRole_Wrong_Type = 2854
Season3_0EquipItem.Toast_Other_Hero_Equiped = 2853
Season3_0EquipItem.Toast_Career_Wrong = 2859
Season3_0EquipItem.Toast_Slot_Lock = 67

function Season3_0EquipItem:checkCanNotEquipWithToast(itemUid)
	if Activity104EquipItemListModel.instance:slotIsLock(Activity104EquipItemListModel.instance.curSelectSlot) then
		GameFacade.showToast(Season3_0EquipItem.Toast_Slot_Lock)
	...
	if itemMO then
		...
		if isTypeWrong then
			if Activity104EquipItemListModel.instance.curPos == Activity104EquipItemListModel.MainCharPos then
				GameFacade.showToast(Season3_0EquipItem.Toast_MainRole_Wrong_Type)
			else
				GameFacade.showToast(Season3_0EquipItem.Toast_Wrong_Type)
			end
		...
		if isCareerWrong then
			GameFacade.showToast(Season3_0EquipItem.Toast_Career_Wrong)
...
function Season3_0EquipItem:onDestroyView()
	if self.icon then
