...
function Season3_0SettlementView:_setStages()
	...
	for i = 1, 7 do
		self:refreshStageGO(self["_gostageitem" .. i], i, stage, maxStage)
	end
end
	...
	if not is_replay then
		...
		if equip104_dic[character_uid] then
			...
			if equip_id and equip_id ~= "0" then
				...
				self:openSubView(Season3_0CelebrityCardGetItem, Season3_0CelebrityCardItem.AssetPath, self._gosupercard, equip_id, true)
			end
		end
	else
