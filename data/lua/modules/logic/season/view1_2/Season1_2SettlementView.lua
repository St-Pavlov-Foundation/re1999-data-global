...
slot0 = class("Season1_2SettlementView", BaseViewExtended)
...
function slot0._showGetCardView(slot0)
	if slot0.equip_cards then
		...
		slot0._showTipsFlow:addWork(Season1_2SettlementTipsWork.New())
...
function slot0.onOpen(slot0)
	...
	if slot0._config then
		slot0._simagedecorate6:LoadImage(ResUrl.getV1A2SeasonIcon(string.format("icon/%s.png", slot0._config.stagePicture)))
	end
...
function slot0._onRewardItemShow(slot0, slot1, slot2, slot3)
	if slot2.materilType == MaterialEnum.MaterialType.EquipCard then
		Season1_2CelebrityCardItem.New():init(gohelper.findChild(gohelper.findChild(slot1, "go_prop"), "cardicon"), slot2.materilId)
...
function slot0._onHeroItemLoaded(slot0)
	...
	if not FightModel.instance.curFightModel then
		...
		for slot11 = 1, 4 do
			if slot12 ~= "0" then
				table.insert(slot0._heroList, slot0:openSubView(Season1_2SettlementHeroItem, slot0._hero_obj_list[slot11], nil, slot4, slot12, slot5[slot11] ~= "0" and slot6[slot12] and slot6[slot12].equipUid, slot12 ~= "0" and slot7[slot12] and slot7[slot12].equipUid))
			end
		end
		...
		if slot7["-100000"] and slot7[slot8].equipUid and slot7[slot8].equipUid[1] and slot9 ~= "0" then
			...
			slot0:openSubView(Season1_2CelebrityCardGetItem, Season1_2CelebrityCardItem.AssetPath, slot0._gosupercard, slot9, true)
		end
	else
...
function slot0._onGetFightRecordGroupReply(slot0, slot1)
	for slot5 = 1, 4 do
		...
		if slot7 ~= "0" then
			table.insert(slot0._heroList, slot0:openSubView(Season1_2SettlementHeroItem, slot0._hero_obj_list[slot5], nil, true, slot7, slot1.replay_equip_data[slot7], slot1.replay_activity104Equip_data[slot7]))
		end
	end
	...
	if slot1.replay_activity104Equip_data["-100000"] then
		...
		slot0:openSubView(Season1_2CelebrityCardGetItem, Season1_2CelebrityCardItem.AssetPath, slot0._gosupercard, nil, , slot3)
	end
end
...
function slot0._onAwardItemShow(slot0, slot1, slot2, slot3)
	...
	if slot2.materilType == MaterialEnum.MaterialType.EquipCard then
		Season1_2CelebrityCardItem.New():init(gohelper.findChild(slot1, "cardicon"), slot2.materilId)
