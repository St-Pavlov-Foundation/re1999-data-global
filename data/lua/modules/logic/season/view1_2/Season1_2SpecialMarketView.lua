...
slot0 = class("Season1_2SpecialMarketView", BaseView)
...
function slot0._refreshLevel(slot0)
	for slot5 = 1, Activity104Model.instance:getMaxSpecialLayer() do
		if not slot0._showLvItems[slot5] then
			slot0._showLvItems[slot5] = Season1_2SpecialMarketShowLevelItem.New()
	...
	slot0._simageleveldecorate:LoadImage(ResUrl.getV1A2SeasonIcon("icon/" .. SeasonConfig.instance:getSeasonSpecialCo(Activity104Model.instance:getCurSeasonId(), slot0._layer).icon .. ".png"))
end
...
function slot0._refreshInfo(slot0)
	...
	slot0._simagestageicon:LoadImage(ResUrl.getV1A2SeasonIcon(string.format("icon/ct_%s.png", slot1.icon)))
