...
slot0 = class("Season1_2HeroGroupFightView", BaseView)
slot1 = 1
...
function slot0._initData(slot0)
	...
	NavigateMgr.instance:addEscape(ViewName.Season1_2HeroGroupFightView, slot0._onEscapeBtnClick, slot0)
...
function slot0._refreshMainCard(slot0)
	slot3 = Activity104EquipItemListModel.MainCharPos
	slot5 = 0

	if HeroGroupModel.instance:getCurGroupMO() then
		if slot1.isReplay and slot1.replay_activity104Equip_data then
			slot5 = slot1.replay_activity104Equip_data["-100000"][1].equipId
		end

		if not slot1.isReplay and slot1.activity104Equips and slot1.activity104Equips[slot3] then
			slot5 = Activity104Model.instance:getItemIdByUid(slot1:getAct104PosEquips(slot3):getEquipUID(slot4))
		end
	end
	...
	if slot5 ~= 0 then
		if not slot0._superCardItem then
			slot0._superCardItem = Season1_2CelebrityCardItem.New()

			slot0._superCardItem:init(slot0._gosupercardpos, slot5)
		else
			gohelper.setActive(slot0._superCardItem.go, true)
			slot0._superCardItem:reset(slot5)
		end
	...
	slot6 = Activity104Model.instance:getAct104CurLayer()
	...
	if slot1.isReplay and DungeonModel.instance.curSendEpisodeId and DungeonConfig.instance:getEpisodeCO(DungeonModel.instance.curSendEpisodeId).type == DungeonEnum.EpisodeType.Season then
		slot6 = Activity104Model.instance:getBattleFinishLayer()
	end

	slot8 = Activity104Model.instance:isSeasonLayerPosUnlock(nil, Activity104Model.instance:getSeasonCurSnapshotSubId(), slot6, slot4, slot3)

	gohelper.setActive(slot0._gosupercardlock, not slot8)
	gohelper.setActive(slot0._gosupercardempty, slot8)
end
...
function slot0._refreshUI(slot0)
	...
	slot0:_refreshCloth()
	slot0:_refreshMainCard()
	slot0.viewContainer:setNavigateOverrideClose(slot0.openSeason1_2MainView, slot0)
end
...
function slot0._checkEquipClothSkill(slot0)
	...
	for slot6, slot7 in ipairs(PlayerClothModel.instance:getList()) do
		if PlayerClothModel.instance:hasCloth(slot7.id) then
			slot9 = Activity104Model.instance:getSeasonCurSnapshotSubId(Activity104Model.instance:getCurSeasonId())
