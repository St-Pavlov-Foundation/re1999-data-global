...
slot0 = class("Season1_2FightCardItem", UserDataDispose)
...
function slot0._setName(slot0)
	if slot0.isMainRole then
		...
		slot0.txtName.text = formatLuaLang("seasoncardnames", HeroModel.instance:getById(slot0.heroUid) and slot1.config and slot1.config.name or "")
	end
end
...
function slot0._setHead(slot0)
	...
	if not slot0.isMainRole and HeroModel.instance:getById(slot0.heroUid) and slot2.skin then
...
function slot0._setCard(slot0, slot1)
	if not slot0.cardItem then
		slot0.cardItem = Season1_2CelebrityCardItem.New()
