...
function Season1_6TaskItem:_singleActiveEnTxt(index)
	local isEN = LangSettings.instance:isEn()

	for i, go in ipairs(self._enGOList) do
		gohelper.setActive(go, not isEN and i == index)
	end
end
...
function Season1_6TaskItem:ctor(go, parentScrollGO)
	self._enGOList = self:getUserDataTb_()

	table.insert(self._enGOList, gohelper.findChild(go, "en1"))
	table.insert(self._enGOList, gohelper.findChild(go, "en2"))
	table.insert(self._enGOList, gohelper.findChild(go, "en3"))
	self:__onInit()
...
function Season1_6TaskItem:refreshDesc(data)
	local config = data.config
	local bgIdx = config.bgType == 1 and 3 or 1
	local bgStr = string.format("tap%s.png", bgIdx)

	self._simageBg:LoadImage(ResUrl.getSeasonIcon(bgStr))

	self._txtDesc.text = config.desc

	self:_singleActiveEnTxt(bgIdx)
end
