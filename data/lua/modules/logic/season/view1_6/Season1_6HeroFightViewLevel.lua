...
slot0 = class("Season1_6HeroFightViewLevel", BaseView)
...
function slot0._refreshTarget(slot0)
	if DungeonConfig.instance:getEpisodeCO(slot0._episodeId).type == DungeonEnum.EpisodeType.SeasonRetail then
	...
	slot7 = slot4 and DungeonEnum.StarType.Normal <= slot4.star and slot5
	slot8 = slot4 and DungeonEnum.StarType.Advanced <= slot4.star and slot5
	slot9 = false

	gohelper.setActive(slot0._gonormalfinish, slot7)
	gohelper.setActive(slot0._gonormalunfinish, not slot7)
	ZProj.UGUIHelper.SetColorAlpha(slot0._txtnormalcondition, slot7 and 1 or 0.63)
	...
	if not string.nilorempty(slot6) then
		...
		gohelper.setActive(slot0._goplatinumfinish, slot8)
		gohelper.setActive(slot0._goplatinumunfinish, not slot8)
		ZProj.UGUIHelper.SetColorAlpha(slot0._txtplatinumcondition, slot8 and 1 or 0.63)
	end

	gohelper.setActive(slot0._goplace, false)
	slot0:_showStar(slot4, slot6, slot7, slot8, slot9)
end
