-- chunkname: @modules/logic/season/view1_6/Season1_6EquipItem.lua

module("modules.logic.season.view1_6.Season1_6EquipItem", package.seeall)

local Season1_6EquipItem = class("Season1_6EquipItem", ListScrollCellExtend)

function Season1_6EquipItem:init(go)
	Season1_6EquipItem.super.init(self, go)
...
function Season1_6EquipItem:addEvents()
	return
end

function Season1_6EquipItem:removeEvents()
	return
end

function Season1_6EquipItem:onUpdateMO(mo)
	self._mo = mo
	self._uid = mo.id
...
Season1_6EquipItem.ColumnCount = 6
Season1_6EquipItem.AnimRowCount = 4
Season1_6EquipItem.OpenAnimTime = 0.06
Season1_6EquipItem.OpenAnimStartTime = 0.05

function Season1_6EquipItem:checkPlayAnim()
...
function Season1_6EquipItem:onDelayPlayOpen()
	TaskDispatcher.cancelTask(self.onDelayPlayOpen, self)
	self._animator:Play("open", 0, 0)
...
function Season1_6EquipItem:refreshUI()
...
function Season1_6EquipItem:refreshDeckCount()
	local itemUid = self._uid
...
function Season1_6EquipItem:refreshEquipedHero(heroUid)
...
function Season1_6EquipItem:refreshIcon(itemId, itemUid)
...
function Season1_6EquipItem:checkCreateIcon()
...
function Season1_6EquipItem:setRoleIconDark(value)
	if value then
...
function Season1_6EquipItem:onClickSelf()
	local itemUid = self._uid
...
function Season1_6EquipItem:checkClickNew(itemUid)
...
Season1_6EquipItem.Toast_Same_Card = 2851
Season1_6EquipItem.Toast_Wrong_Type = 2852
Season1_6EquipItem.Toast_MainRole_Wrong_Type = 2854
Season1_6EquipItem.Toast_Other_Hero_Equiped = 2853
Season1_6EquipItem.Toast_Career_Wrong = 2859
Season1_6EquipItem.Toast_Slot_Lock = 67

function Season1_6EquipItem:checkCanNotEquipWithToast(itemUid)
	if Activity104EquipItemListModel.instance:slotIsLock(Activity104EquipItemListModel.instance.curSelectSlot) then
		GameFacade.showToast(Season1_6EquipItem.Toast_Slot_Lock)
	...
	if itemMO then
		...
		if isTypeWrong then
			if Activity104EquipItemListModel.instance.curPos == Activity104EquipItemListModel.MainCharPos then
				GameFacade.showToast(Season1_6EquipItem.Toast_MainRole_Wrong_Type)
			else
				GameFacade.showToast(Season1_6EquipItem.Toast_Wrong_Type)
			end
		...
		if isCareerWrong then
			GameFacade.showToast(Season1_6EquipItem.Toast_Career_Wrong)
...
function Season1_6EquipItem:onDestroyView()
	if self.icon then
