...
slot0 = class("Season1_6MainScene", BaseView)
...
function slot0._initSceneRootNode(slot0)
	slot0._sceneRoot = UnityEngine.GameObject.New("Season1_6MainScene")
...
function slot0._loadSceneFinish(slot0)
	...
	MainCameraMgr.instance:addView(ViewName.Season1_6MainView, slot0.autoInitMainViewCamera, slot0.resetCamera, slot0)
end
...
function slot0.showLevelObjs(slot0, slot1)
	...
	if ViewMgr.instance:getContainer(ViewName.Season1_6MainView) then
		slot5:stopUI(false)
	end
end
...
function slot0.focusRole(slot0, slot1)
	...
	if not gohelper.findChild(slot0[string.format("_gorole%s", slot1)], "vx") then
		return
	end
