...
slot0 = class("SeasonSettlementHeroItem", BaseViewExtended)
...
function slot0.onRefreshViewParam(slot0, slot1, slot2, slot3, slot4)
...
function slot0._showHeroIcon(slot0, slot1)
	if HeroModel.instance:getById(slot1) then
		slot0._simageheroicon:LoadImage(ResUrl.getHeadIconMiddle(FightConfig.instance:getSkinCO(slot2.skin).retangleIcon))
		UISpriteSetMgr.instance:setCommonSprite(slot0._imagecareer, "lssx_" .. tostring(slot2.config.career))
	else
...
function slot0._showEquip104(slot0, slot1, slot2, slot3, slot4)
	...
	slot0:openSubView(SeasonCelebrityCardGetItem, "ui/viewres/season/seasoncelebritycarditem.prefab", slot3 <= 1 and slot0._gosingle or slot0["_gocard" .. slot1], slot2, nil, slot4)
end
