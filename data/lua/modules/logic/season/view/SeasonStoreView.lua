...
slot0 = class("SeasonStoreView", BaseView)
...
function slot0._editableInitView(slot0)
	slot0._simagebg:LoadImage(ResUrl.getSeasonIcon("full/img_saiji_store_bg.png"))
...
function slot0.refreshStoreContent(slot0)
	...
	for slot6, slot7 in pairs(ActivityStoreConfig.instance:getActivityStoreGroupDict(slot0.actId)) do
	...
	for slot6 = 1, math.max(#slot2, slot6) do
		if not slot0.storeItemList[slot6] then
			slot0.storeItemList[slot6] = SeasonStoreItem.New(slot0:getItemGo(slot6))
		end
...
function slot0.refreshTime(slot0)
	slot3, slot4, slot5 = TimeUtil.secondsToDDHHMMSS(ActivityModel.instance:getActMO(slot0.actId):getRealEndTimeStamp() - ServerTime.now())
	slot0._txttime.text = string.format(luaLang("versionactivitystoreview_remaintime"), slot3, slot4, slot5)
end
