...
local var_0_0 = class("VersionActivity3_0EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function var_0_0.afterSetData(arg_3_0)
	...
	if not arg_3_0.activityCo then
		...
		logError("VersionActivity3_0EnterViewTabItemBase.afterSetData error, no act config, actId:%s", arg_3_0.actId)
	...
	if not string.nilorempty(var_3_1) then
		UISpriteSetMgr.instance:setV3a0MainActivitySprite(arg_3_0.imageSelectTabIcon, var_3_1)
	end

	local var_3_2 = var_3_0[2]

	if not string.nilorempty(var_3_2) then
		UISpriteSetMgr.instance:setV3a0MainActivitySprite(arg_3_0.imageUnSelectTabIcon, var_3_2)
	end
...
function var_0_0.childRefreshSelect(arg_4_0)
	...
	if not arg_4_0.isSelect then
		arg_4_0.animator:Play("idle", 0, 0)
	end
end
...
function var_0_0.refreshNormalTag(arg_8_0)
	...
	if VersionActivity3_0EnterHelper.GetIsShowTabRemainTime(arg_8_0.actId) and var_8_0 then
		if var_8_0:getRealEndTimeStamp() - ServerTime.now() > VersionActivity3_0Enum.MaxShowTimeOffset then
			return
		end
