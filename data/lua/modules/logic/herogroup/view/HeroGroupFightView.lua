...
function HeroGroupFightView:onInitView()
	...
	self._goclothbg = gohelper.findChild(self.viewGO, "#go_container/btnContain/btnCloth/bg")
	self._txtclothname = gohelper.findChildText(self.viewGO, "#go_container/btnContain/btnCloth/bg/#txt_clothName")
	self._txtclothnameen = gohelper.findChildText(self.viewGO, "#go_container/btnContain/btnCloth/bg/#txt_clothName/#txt_clothNameEn")
...
function HeroGroupFightView:_editableInitView()
	...
	if self._goclothbg then
		self._iconGO = self:getResInst(self.viewContainer:getSetting().otherRes[1], self._goclothbg.gameObject)

		local comp = gohelper.onceAddComponent(self._iconGO, typeof(UnityEngine.UI.LayoutElement))

		comp.ignoreLayout = true
		self._iconGO.transform.anchorMin = Vector2.up
		self._iconGO.transform.anchorMax = Vector2.up
		self._iconGO.transform.pivot = Vector2.up

		recthelper.setAnchor(self._iconGO.transform, -90, 70)
	end
...
function HeroGroupFightView:_refreshReplay()
	if self._chapterConfig.type == DungeonEnum.ChapterType.WeekWalk then
		...
		recthelper.setWidth(self._goReplayBtn.transform, self._replayMode and 500 or 83)
...
function HeroGroupFightView:_switchReplayMul()
	...
	local replayreadyTip = gohelper.findChildTextMesh(self._goreplayready, "tip")
	local fontColor = GameConfig:GetCurLangType() == LangSettings.zh and Color.white or SLFramework.UGUI.GuiHelper.ParseColor("#D6B5B5")

	replayreadyTip.color = fontColor
end
