...
function slot0._init_overseas(slot0)
	slot0._tagTranList = slot0:getUserDataTb_()

	for slot5 = 1, slot0._tagTr.childCount do
		table.insert(slot0._tagTranList, slot0._tagTr:GetChild(slot5 - 1))
	end
end

function slot0._refreshTagsPos(slot0)
	slot1 = 160

	for slot6, slot7 in ipairs(slot0._tagTranList) do
		if slot7.gameObject.activeSelf then
			recthelper.setAnchorY(slot7, slot1)
...
function slot0.setParent(slot0, slot1)
	...
	for slot5, slot6 in ipairs(slot0._tagTranList) do
		slot6:SetParent(slot1, true)

		slot6.anchorMin = Vector2.New(0, 1)
		slot6.anchorMax = Vector2.New(0, 1)
		slot6.pivot = Vector2(0, 0)
	end
...
function slot0.initEquips(slot0, slot1)
	...
	if slot0.isLock or slot0.isAid or slot0.isRoleNumLock or not OpenModel.instance:isFunctionUnlock(OpenEnum.UnlockFunc.Equip) and not slot0.trialCO and not HeroGroupTrialModel.instance:haveTrialEquip() then
		...
		elseif slot5 then
			...
			slot0._equip.equiptxtlv.text = luaLang("level") .. slot0.trialCO.equipLv
