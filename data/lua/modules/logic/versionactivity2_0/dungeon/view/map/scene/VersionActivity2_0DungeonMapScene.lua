...
slot0 = class("VersionActivity2_0DungeonMapScene", BaseView)
...
function slot0.addEvents(slot0)
	...
	slot0:addEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.FocusElement, slot0.onFocusElement, slot0)
	slot0:addEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.ManualClickElement, slot0.manualClickElement, slot0)
...
function slot0.removeEvents(slot0)
	...
	slot0:removeEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.FocusElement, slot0.onFocusElement, slot0)
	slot0:removeEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.ManualClickElement, slot0.manualClickElement, slot0)
...
function slot0._onScreenResize(slot0)
	if slot0._sceneGo then
		CameraMgr.instance:getMainCamera().orthographicSize = VersionActivity2_0DungeonEnum.DungeonMapCameraSize * GameUtil.getAdapterScale()
...
function slot0.onActivityDungeonMoChange(slot0)
	slot0:refreshMap(VersionActivity2_0DungeonModel.instance:getMapNeedTweenState())
end
...
function slot0._initMapRootNode(slot0)
	slot0._sceneRoot = UnityEngine.GameObject.New(VersionActivity2_0DungeonEnum.SceneRootName)
...
function slot0._initCamera(slot0)
	...
	slot1.orthographicSize = VersionActivity2_0DungeonEnum.DungeonMapCameraSize * GameUtil.getAdapterScale()
end

function slot0.refreshMap(slot0, slot1, slot2)
	slot0._mapCfg = slot2 or VersionActivity2_0DungeonConfig.instance:getEpisodeMapConfig(slot0.activityDungeonMo.episodeId)
	...
	if slot0._mapCfg.id == slot0._lastLoadMapId then
		...
		if VersionActivity2_0DungeonModel.instance:getOpenGraffitiEntranceState() then
			slot0:_setGraffitiMapSceneCanvas()
		else
			slot0:_setMapPos()
		end
	else
...
function slot0._initScene(slot0)
	...
	slot11 = VersionActivity2_0DungeonEnum.DungeonMapCameraSize / (CameraMgr.instance:getUICamera() and slot9.orthographicSize or 5)
...
function slot0._addSceneCanvas(slot0)
	slot1 = slot0._sceneCanvas
	slot0._graffitiCanvasGo = gohelper.clone(slot0._mapLoader:getAssetItem(slot1):GetResource(slot1), slot0._sceneGo, "graffitiMapSceneCanvas")
	slot0._graffitiCanvasGo:GetComponent("Canvas").worldCamera = CameraMgr.instance:getMainCamera()

	slot0:_setGraffitiMapSceneCanvas()
end

function slot0._setGraffitiMapSceneCanvas(slot0)
	Activity161Controller.instance:dispatchEvent(Activity161Event.SetGraffitiMapSceneCanvas, {
		mapConfig = slot0._mapCfg,
		canvasGO = slot0._graffitiCanvasGo,
		sceneGO = slot0._sceneGo
	})
	slot0:_setMapPos()
end
...
function slot0.directSetScenePos(slot0, slot1)
	...
	VersionActivity2_0DungeonController.instance:dispatchEvent(VersionActivity2_0DungeonEvent.OnMapPosChanged, slot0._scenePos, slot0.needTween)
