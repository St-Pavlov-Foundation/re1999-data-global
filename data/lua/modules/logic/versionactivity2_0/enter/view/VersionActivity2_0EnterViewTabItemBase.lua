...
slot0 = class("VersionActivity2_0EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function slot0._editableInitView(slot0)
	...
	slot0.goTagNewAct = gohelper.findChild(slot0.go, "#go_tag/#go_newact")
	slot0.goTagNewLevel = gohelper.findChild(slot0.go, "#go_tag/#go_newlevel")
	slot0.goTagTime = gohelper.findChild(slot0.go, "#go_tag/#go_time")
	slot0.goTagLock = gohelper.findChild(slot0.go, "#go_tag/#go_lock")
...
function slot0.afterSetData(slot0)
	...
	if not slot0.activityCo then
		...
		logError("VersionActivity2_0EnterViewTabItemBase.afterSetData error, no act config, actId:%s", slot0.actId)

		return
	end

	if not string.nilorempty(string.split(slot0.activityCo.tabBgPath, "#")[1]) then
		UISpriteSetMgr.instance:setV2a0MainActivitySprite(slot0.imageSelectTabIcon, slot2)
	end

	if not string.nilorempty(slot1[2]) then
		UISpriteSetMgr.instance:setV2a0MainActivitySprite(slot0.imageUnSelectTabIcon, slot3)
	end
...
function slot0.refreshNormalTag(slot0)
	...
	if VersionActivity2_0EnterHelper.GetIsShowTabRemainTime(slot0.actId) and slot1 then
		if VersionActivity2_0Enum.MaxShowTimeOffset < slot1:getRealEndTimeStamp() - ServerTime.now() then
			return
		end
