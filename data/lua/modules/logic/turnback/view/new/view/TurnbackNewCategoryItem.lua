-- chunkname: @modules/logic/turnback/view/new/view/TurnbackNewCategoryItem.lua

module("modules.logic.turnback.view.new.view.TurnbackNewCategoryItem", package.seeall)

local TurnbackNewCategoryItem = class("TurnbackNewCategoryItem", ListScrollCell)

function TurnbackNewCategoryItem:init(go)
	self.go = go
...
function TurnbackNewCategoryItem:addEventListeners()
	self._itemClick:AddClickListener(self._onItemClick, self)
end

function TurnbackNewCategoryItem:removeEventListeners()
	self._itemClick:RemoveClickListener()
end

function TurnbackNewCategoryItem:_onItemClick()
...
function TurnbackNewCategoryItem:onUpdateMO(mo)
	self._mo = mo
...
function TurnbackNewCategoryItem:_refreshItem()
...
function TurnbackNewCategoryItem:_checkCustomShowRedDotData(redDotIcon)
	TurnbackController.instance:_checkCustomShowRedDotData(redDotIcon, self._mo.id)
end

function TurnbackNewCategoryItem:_setLoginReddOtData(id)
	local signStr = self.curTurnbackId .. "_" .. id

	TimeUtil.setDayFirstLoginRed(signStr)
end

function TurnbackNewCategoryItem:playEnterAnim()
...
function TurnbackNewCategoryItem:onDestroy()
	return
end

return TurnbackNewCategoryItem
