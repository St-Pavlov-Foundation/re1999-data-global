...
slot0 = class("ActMarcusTaskListModel", ListScrollModel)
...
function slot0.refreshData(slot0)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.RoleActivity) ~= nil then
		for slot8, slot9 in pairs(RoleActivityConfig.instance:getActicityTaskList(VersionActivity1_7Enum.ActivityId.Marcus)) do
			slot10 = ActMarcusTaskMO.New()
	...
	if slot3 > 1 then
		slot4 = ActMarcusTaskMO.New()
		slot4.id = ActMarcusEnum.TaskMOAllFinishId
		slot4.activityId = VersionActivity1_7Enum.ActivityId.Marcus
...
function slot0.getSortIndex(slot0)
	if slot0.id == ActMarcusEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == ActMarcusEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= ActMarcusEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(ActMarcusEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= ActMarcusEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
