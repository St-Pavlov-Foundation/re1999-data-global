...
slot0 = class("ActIsoldeTaskListModel", ListScrollModel)
...
function slot0.refreshData(slot0)
	...
	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.RoleActivity) ~= nil then
		for slot8, slot9 in pairs(RoleActivityConfig.instance:getActicityTaskList(VersionActivity1_7Enum.ActivityId.Isolde)) do
			slot10 = ActIsoldeTaskMO.New()
	...
	if slot3 > 1 then
		slot4 = ActIsoldeTaskMO.New()
		slot4.id = ActIsoldeEnum.TaskMOAllFinishId
		slot4.activityId = VersionActivity1_7Enum.ActivityId.Isolde
...
function slot0.getSortIndex(slot0)
	if slot0.id == ActIsoldeEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == ActIsoldeEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= ActIsoldeEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(ActIsoldeEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= ActIsoldeEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
