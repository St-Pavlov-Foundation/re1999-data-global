...
slot0 = class("ActIsoldeTaskView", BaseView)
...
function slot0._onFinishTask(slot0, slot1)
	if ActIsoldeTaskListModel.instance:getById(slot1) then
		ActIsoldeTaskListModel.instance:refreshData()
	end
end

function slot0._showLeftTime(slot0)
	slot0._txtLimitTime.text = ActivityHelper.getActivityRemainTimeStr(VersionActivity1_7Enum.ActivityId.Isolde)
end
