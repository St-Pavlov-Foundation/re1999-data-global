-- chunkname: @modules/logic/versionactivity1_7/isolde/view/IsoldeFightItem.lua

module("modules.logic.versionactivity1_7.isolde.view.IsoldeFightItem", package.seeall)

local IsoldeFightItem = class("IsoldeFightItem", LuaCompBase)

function IsoldeFightItem:init(go)
	self.viewGO = go
...
function IsoldeFightItem:addEventListeners()
...
function IsoldeFightItem:removeEventListeners()
	self._btnclick:RemoveClickListener()
	self._btnNormal:RemoveClickListener()
	self._btnHard:RemoveClickListener()
end

function IsoldeFightItem:onDestroy()
	return
end

function IsoldeFightItem:_btnOnClick()
	...
	ActIsoldeController.instance:dispatchEvent(ActIsoldeEvent.FightItemClick, self.index)
end

function IsoldeFightItem:_btnOnNormal()
	self:enterFight(self.config)
end

function IsoldeFightItem:_btnOnHard()
	local unLock = ActIsoldeModel.instance:isLevelUnlock(self.hardConfig.id)
...
function IsoldeFightItem:setParam(co, _index)
	self.config = co
	self.id = co.id
...
function IsoldeFightItem:_refreshUI()
	self:refreshStatus()
...
function IsoldeFightItem:refreshStatus()
	self.unlock = ActIsoldeModel.instance:isLevelUnlock(self.id)

	local hardUnlock = ActIsoldeModel.instance:isLevelUnlock(self.hardConfig.id)
...
function IsoldeFightItem:refreshStar()
	local passNormal = ActIsoldeModel.instance:isLevelPass(self.id)
	local passHard = ActIsoldeModel.instance:isLevelPass(self.hardConfig.id)
...
function IsoldeFightItem:refreshSelect(_index)
...
function IsoldeFightItem:isUnlock()
	return self.unlock
end

function IsoldeFightItem:enterFight(episodeConfig)
