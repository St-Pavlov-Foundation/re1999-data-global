...
slot0 = class("IsoldeFightItem", LuaCompBase)
...
function slot0._btnOnClick(slot0)
	...
	ActIsoldeController.instance:dispatchEvent(ActIsoldeEvent.FightItemClick, slot0.index)
end
...
function slot0._btnOnHard(slot0)
	if not ActIsoldeModel.instance:isLevelUnlock(slot0.hardConfig.id) then
		GameFacade.showToast(ToastEnum.DungeonIsLockNormal)
...
function slot0.refreshStatus(slot0)
	slot0.unlock = ActIsoldeModel.instance:isLevelUnlock(slot0.id)

	gohelper.setActive(slot0._goLock, not slot0.unlock)
	gohelper.setActive(slot0._goHardLock, not ActIsoldeModel.instance:isLevelUnlock(slot0.hardConfig.id))
	slot0:refreshStar()
end

function slot0.refreshStar(slot0)
	slot1 = ActIsoldeModel.instance:isLevelPass(slot0.id)
	slot2 = ActIsoldeModel.instance:isLevelPass(slot0.hardConfig.id)
...
function slot0.enterFight(slot0, slot1)
	...
	if slot1.chapterId and slot3 and slot4 > 0 then
		ActIsoldeModel.instance:setEnterFightIndex(slot0.index)
