...
slot0 = class("ActIsoldeTaskItem", ListScrollCellExtend)
...
function slot0._btnfinishbgOnClick(slot0)
	if ActIsoldeController.instance:delayReward(ActIsoldeEnum.AnimatorTime.TaskReward, slot0._actTaskMO) then
...
function slot0._btngetallOnClick(slot0)
	if ActIsoldeController.instance:delayReward(ActIsoldeEnum.AnimatorTime.TaskReward, slot0._actTaskMO) then
		ActIsoldeController.instance:dispatchEvent(ActIsoldeEvent.OneClickClaimReward, slot0._actTaskMO.activityId)
	end
end
...
function slot0._editableAddEvents(slot0)
	slot0:addEventCb(ActIsoldeController.instance, ActIsoldeEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	slot0:removeEventCb(ActIsoldeController.instance, ActIsoldeEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._actTaskMO and slot0._actTaskMO.activityId == slot1 and (slot0._actTaskMO:alreadyGotReward() or slot0._actTaskMO.id == ActIsoldeEnum.TaskMOAllFinishId) then
...
function slot0.onUpdateMO(slot0, slot1)
	...
	slot0:_refreshUI()
	slot0:_moveByRankDiff(ActIsoldeTaskListModel.instance.instance:getRankDiff(slot1))
end
...
function slot0._moveByRankDiff(slot0, slot1)
	if slot1 and slot1 ~= 0 then
		...
		slot0._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(slot0.viewTrs, 0, ActIsoldeEnum.AnimatorTime.TaskRewardMoveUp)
	end
end
...
function slot0._refreshUI(slot0)
	...
	slot2 = slot1.id ~= ActIsoldeEnum.TaskMOAllFinishId
