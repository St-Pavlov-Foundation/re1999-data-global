...
function AbyssTaskItem:onUpdateMO(mo)
	...
	if index then
		if index < 10 then
			self._txtindex.text = "0" .. tostring(index)
		else
			self._txtindex.text = tostring(index)
		end
	end
...
function AbyssTaskItem:_updateStatus()
	...
	local isGet = self._taskMo and self._taskMo.finishCount >= self._config.maxFinishCount
	...
	if not isGet then
		local isFinish = self._taskMo and self._taskMo.hasFinished
	...
	self._imageprogress.fillAmount = self._taskMo and self._taskMo.progress / self._config.maxProgress or 0
