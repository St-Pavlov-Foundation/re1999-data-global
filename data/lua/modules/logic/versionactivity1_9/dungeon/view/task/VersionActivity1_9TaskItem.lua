-- chunkname: @modules/logic/versionactivity1_9/dungeon/view/task/VersionActivity1_9TaskItem.lua

module("modules.logic.versionactivity1_9.dungeon.view.task.VersionActivity1_9TaskItem", package.seeall)

local VersionActivity1_9TaskItem = class("VersionActivity1_9TaskItem", ListScrollCell)

function VersionActivity1_9TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity1_9TaskItem:addEventListeners()
...
function VersionActivity1_9TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
end

function VersionActivity1_9TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity1_9TaskView)
		end
	end
end

function VersionActivity1_9TaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

VersionActivity1_9TaskItem.FinishKey = "VersionActivity1_9TaskItem FinishKey"

function VersionActivity1_9TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity1_9TaskItem.FinishKey)
...
function VersionActivity1_9TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity1_9TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity1_9TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity1_9Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity1_9TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity1_9TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity1_9TaskItem:refreshNormalUI()
...
function VersionActivity1_9TaskItem:refreshRewardItems()
...
function VersionActivity1_9TaskItem:refreshGetAllUI()
	return
end

function VersionActivity1_9TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity1_9TaskItem:getAnimator()
	return self.animator
end

function VersionActivity1_9TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity1_9TaskItem
