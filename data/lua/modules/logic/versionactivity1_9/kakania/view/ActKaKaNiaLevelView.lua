...
slot0 = class("ActKaKaNiaLevelView", BaseView)
...
function slot0._checkFirstEnter(slot0)
	if not slot0.storyItemList[2]:isUnlock() and PlayerPrefsHelper.getNumber("ActKaKaNiaFirstEnter", 0) == 0 then
...
function slot0._initStageItems(slot0)
	...
	for slot7 = 1, #RoleActivityConfig.instance:getStoryLevelList(slot0.actId) do
		slot0.storyItemList[slot7] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], gohelper.findChild(slot0._gostoryStages, "stage" .. slot7)), KaKaNiaStoryItem, slot0)
	...
	for slot8 = 1, #RoleActivityConfig.instance:getBattleLevelList(slot0.actId) / 2 do
		slot0.fightItemList[slot8] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], gohelper.findChild(slot0._gofightStages, "stage" .. slot8)), KaKaNiaFightItem, slot0)
