-- chunkname: @modules/logic/versionactivity1_9/lucy/view/LucyStoryItem.lua

module("modules.logic.versionactivity1_9.lucy.view.LucyStoryItem", package.seeall)

local LucyStoryItem = class("LucyStoryItem", LuaCompBase)

function LucyStoryItem:init(go)
	self.viewGO = go
...
function LucyStoryItem:addEventListeners()
	self._btnclick:AddClickListener(self._btnOnClick, self)
	self._btnreview:AddClickListener(self._btnOnReview, self)
end

function LucyStoryItem:removeEventListeners()
	self._btnclick:RemoveClickListener()
	self._btnreview:RemoveClickListener()
end

function LucyStoryItem:onDestroy()
	return
end

function LucyStoryItem:_btnOnClick()
...
function LucyStoryItem:_btnOnReview()
	self:_btnOnClick()
end

function LucyStoryItem:setParam(co, _index, _actId)
	self.config = co
	self.id = co.id
	self.actId = _actId
	self.index = _index
...
function LucyStoryItem:_refreshUI()
	self:refreshStatus()
...
function LucyStoryItem:refreshStatus()
...
function LucyStoryItem:onStoryFinished()
