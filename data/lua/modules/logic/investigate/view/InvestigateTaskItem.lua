...
slot0 = class("InvestigateTaskItem", ListScrollCell)
...
slot0.FinishKey = "InvestigateTaskItem FinishKey"
...
function slot0.secondAnimationDone(slot0)
	...
	if slot0.taskMo.getAll then
		...
		TaskRpc.instance:sendFinishTaskRequest(slot0.co.taskId)
	end
end
...
function slot0.refreshNormalUI(slot0)
	slot0.co = lua_investigate_reward.configDict[slot0.taskMo.id]
	...
	if slot0._maxFinishCount <= slot0.taskMo.finishCount then
