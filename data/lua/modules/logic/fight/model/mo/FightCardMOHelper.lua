module("modules.logic.fight.model.mo.FightCardMOHelper", package.seeall)
...
return {
	...
	canCombineCard = function (slot0, slot1)
		...
		if FightEnum.UniversalCard[slot0.skillId] or FightEnum.UniversalCard[slot3] then
			return false
		end

		if slot2 == slot3 then
			if not FightCardModel.instance:getSkillNextLvId(slot0.uid, slot2) then
				return false
			end
		else
			return false
