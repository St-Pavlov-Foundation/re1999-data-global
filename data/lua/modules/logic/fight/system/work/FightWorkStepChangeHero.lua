...
function slot0.isMySide(slot0)
	if FightEntityModel.instance:getById(slot0._fightStepMO.fromId) then
...
function slot0.onStart(slot0)
	...
	if FightHelper.getEntity(slot0.from_id) and slot1.spine:getSpineGO() ~= nil then
		...
		slot2 = FightEntityModel.instance:getById(slot0.from_id)
		...
		if slot2 and slot3 then
			...
			slot0.entityMgr:buildSubSpine(slot2)
		else
			slot0:_startChangeHero()
		end
	end
...
function slot0._startChangeHero(slot0)
	...
	slot0.from_entity_mo = FightEntityModel.instance:getById(slot0.from_id)
	slot0.to_entity = slot0.entityMgr:getEntity(slot0.to_id)
	slot0.to_entity_mo = FightEntityModel.instance:getById(slot0.to_id)
	slot0.to_entity_mo = slot0.to_entity_mo or FightEntityModel.instance:getDeadById(slot0.to_id)
...
function slot0._targetEntityQuitFinish(slot0)
	slot0._from_pos = slot0.from_entity_mo.position
	slot0._jump_pos = slot0.to_entity_mo.position
	slot0._jump_end_pos = {}
	slot0._jump_end_pos.x, slot0._jump_end_pos.y, slot0._jump_end_pos.z = FightHelper.getEntityStandPos(slot0.to_entity_mo)

	if slot0.to_entity then
		slot0.entityMgr:removeUnit(slot0.to_entity:getTag(), slot0.to_entity.id)

		if not slot0.to_entity.isDead then
			slot0.to_entity_mo.position = slot0.from_entity_mo.position

			if FightEntityModel.instance:getSubModel(slot0.to_entity_mo.side):getIndex(FightEntityModel.instance:getById(slot0._fightStepMO.fromId)) then
				slot1:addAt(slot0.to_entity_mo, slot3)
			else
				logError("换替补失败 index = nil")
			end

			FightEntityModel.instance:getModel(slot0.to_entity_mo.side):remove(slot0.to_entity_mo)
		end
...
function slot0._entityEnter(slot0)
	...
	slot0.from_entity_mo.position = slot0._jump_pos

	FightEntityModel.instance:getSubModel(slot0.from_entity_mo.side):remove(slot0.from_entity_mo)
	FightEntityModel.instance:getModel(slot0.from_entity_mo.side):addAtLast(slot0.from_entity_mo)
	...
	if slot0._fightStepMO.actEffectMOs then
		for slot6, slot7 in ipairs(slot0._fightStepMO.actEffectMOs) do
			if slot7.effectType == FightEnum.EffectType.CHANGEHERO then
				slot0.from_entity_mo = slot7.entityMO

				FightEntityModel.instance:removeEntityById(slot0.from_entity_mo.id)
				FightEntityModel.instance:addEntityMO(slot0.from_entity_mo)
...
function slot0._onEntityBornDone(slot0)
	...
	if slot0.from_entity_mo and slot0.from_entity_mo.side == FightEnum.EntitySide.MySide and FightEntityModel.instance:getSubModel(FightEnum.EntitySide.MySide):getList()[1] then
	...
	if FightEntityModel.instance:getById(slot0.to_id) then
		slot1:onChangeHero()
	end
