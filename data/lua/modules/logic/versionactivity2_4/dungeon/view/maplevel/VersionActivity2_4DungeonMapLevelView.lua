...
function slot0._buildEpisodeName_overseas(slot0, slot1, slot2)
	slot4 = slot0.showEpisodeCo.id
	slot5 = false
	slot6 = false
	...
	if slot6 then
		recthelper.setHeight(slot0._txtmapName.transform, 140)

		return string.format("<size=74>%s</size>%s", slot1, slot2)
	elseif slot5 then
		recthelper.setHeight(slot7, 110)

		return string.format("<size=80>%s</size>%s", slot1, slot2)
	else
		recthelper.setHeight(slot7, 160)

		return string.format("<size=112>%s</size>%s", slot1, slot2)
	end
end
...
function slot0.buildEpisodeName(slot0, slot1)
	...
	return slot0:buildColorText(slot0:_buildEpisodeName_overseas(slot3, slot4), slot0.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard and "#cfccc9" or "#cfccc9")
end
