-- chunkname: @modules/logic/versionactivity2_4/wuerlixi/model/WuErLiXiTaskListModel.lua

module("modules.logic.versionactivity2_4.wuerlixi.model.WuErLiXiTaskListModel", package.seeall)

local WuErLiXiTaskListModel = class("WuErLiXiTaskListModel", ListScrollModel)

function WuErLiXiTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity180)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = WuErLiXiConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = WuErLiXiTaskMo.New()
	...
	if rewardCount > 1 then
		local allMO = WuErLiXiTaskMo.New()
	...
	table.sort(dataList, WuErLiXiTaskListModel.sortMO)
...
function WuErLiXiTaskListModel.sortMO(objA, objB)
	local sidxA = WuErLiXiTaskListModel.getSortIndex(objA)
	local sidxB = WuErLiXiTaskListModel.getSortIndex(objB)
...
function WuErLiXiTaskListModel.getSortIndex(objA)
...
function WuErLiXiTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function WuErLiXiTaskListModel:getRankDiff(mo)
...
function WuErLiXiTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function WuErLiXiTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, WuErLiXiTaskListModel.sortMO)
...
function WuErLiXiTaskListModel:getGotRewardCount(moList)
...
WuErLiXiTaskListModel.instance = WuErLiXiTaskListModel.New()

return WuErLiXiTaskListModel
