-- chunkname: @modules/logic/versionactivity2_8/dungeontaskstore/view/task/VersionActivity2_8TaskItem.lua

module("modules.logic.versionactivity2_8.dungeontaskstore.view.task.VersionActivity2_8TaskItem", package.seeall)

local VersionActivity2_8TaskItem = class("VersionActivity2_8TaskItem", ListScrollCell)

function VersionActivity2_8TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity2_8TaskItem:addEventListeners()
	...
	VersionActivity2_8DungeonTaskStoreController.instance:registerCallback(VersionActivity2_8DungeonEvent.OnClickAllTaskFinish, self._OnClickAllTaskFinish, self)
end

function VersionActivity2_8TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
	VersionActivity2_8DungeonTaskStoreController.instance:unregisterCallback(VersionActivity2_8DungeonEvent.OnClickAllTaskFinish, self._OnClickAllTaskFinish, self)
end

function VersionActivity2_8TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity2_8TaskView)
		end
	end
end

function VersionActivity2_8TaskItem:_btnFinishAllOnClick()
	VersionActivity2_8DungeonTaskStoreController.instance:dispatchEvent(VersionActivity2_8DungeonEvent.OnClickAllTaskFinish)
end

function VersionActivity2_8TaskItem:_OnClickAllTaskFinish()
	if self.taskMo then
...
VersionActivity2_8TaskItem.FinishKey = "VersionActivity2_8TaskItem FinishKey"

function VersionActivity2_8TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity2_8TaskItem.FinishKey)
...
function VersionActivity2_8TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity2_8TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity2_8TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity2_8DungeonTaskStoreController.instance:getModuleConfig().Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity2_8TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity2_8TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity2_8TaskItem:refreshNormalUI()
...
function VersionActivity2_8TaskItem:refreshRewardItems()
...
function VersionActivity2_8TaskItem:refreshGetAllUI()
	return
end

function VersionActivity2_8TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity2_8TaskItem:getAnimator()
	return self.animator
end

function VersionActivity2_8TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity2_8TaskItem
