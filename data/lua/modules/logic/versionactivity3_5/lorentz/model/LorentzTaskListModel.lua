-- chunkname: @modules/logic/versionactivity3_5/lorentz/model/LorentzTaskListModel.lua

module("modules.logic.versionactivity3_5.lorentz.model.LorentzTaskListModel", package.seeall)

local LorentzTaskListModel = class("LorentzTaskListModel", ListScrollModel)

function LorentzTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity220)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = LorentzConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = LorentzTaskMo.New()
	...
	table.sort(dataList, LorentzTaskListModel.sortMO)
...
function LorentzTaskListModel.sortMO(objA, objB)
	local sidxA = LorentzTaskListModel.getSortIndex(objA)
	local sidxB = LorentzTaskListModel.getSortIndex(objB)
...
function LorentzTaskListModel.getSortIndex(objA)
...
function LorentzTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function LorentzTaskListModel:getRankDiff(mo)
...
function LorentzTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function LorentzTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, LorentzTaskListModel.sortMO)
...
function LorentzTaskListModel:getGotRewardCount(moList)
...
LorentzTaskListModel.instance = LorentzTaskListModel.New()

return LorentzTaskListModel
