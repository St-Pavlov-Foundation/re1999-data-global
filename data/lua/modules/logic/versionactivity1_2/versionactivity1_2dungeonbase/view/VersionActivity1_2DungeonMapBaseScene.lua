...
slot0 = class("VersionActivity1_2DungeonMapBaseScene", BaseViewExtended)
...
function slot0._initMapRootNode(slot0)
	slot0._sceneRoot = UnityEngine.GameObject.New("VersionActivity1_2DungeonMapBaseScene")
	slot3, slot4, slot5 = transformhelper.getLocalPos(CameraMgr.instance:getMainCameraTrs().parent)

	transformhelper.setLocalPos(slot0._sceneRoot.transform, 0, slot4, 20)
...
function slot0.changeMap(slot0, slot1, slot2)
	slot0._forcePos = slot2

	if slot0._mapCfg and slot0._mapCfg.id == slot1.id then
		return
	end
...
function slot0._loadSceneFinish(slot0)
	...
	VersionActivity1_2DungeonController.instance:dispatchEvent(VersionActivity1_2DungeonEvent.guideOnLoadSceneFinish, slot0._mapCfg.id)

	slot0.goFogEffect = gohelper.clone(slot0._mapLoader:getAssetItem(slot0._fogUrl):GetResource(slot0._fogUrl), slot0._sceneGo)

	transformhelper.setLocalPos(slot0.goFogEffect.transform, 0, 0, -5)

	slot0.goRainEffect = gohelper.findChild(slot0._sceneGo, "SceneEffect")

	slot0:changeDungeonMode(slot0.isHardDungeonMode)
	slot0:_initScene()
	slot0:_initCanvas()
	slot0:_addAllAudio()
	slot0:_onLoadSceneFinish()
end
...
function slot0.buildLoadRes(slot0, slot1, slot2)
	...
	table.insert(slot1, "scenes/m_s08_hddt/prefab/s08_hddt_hd_01_fog.prefab")
end
...
function slot0._rebuildScene(slot0)
	slot0:initMap(slot0._tempMapCfg)
...
function slot0.changeDungeonMode(slot0, slot1)
	if slot1 then
