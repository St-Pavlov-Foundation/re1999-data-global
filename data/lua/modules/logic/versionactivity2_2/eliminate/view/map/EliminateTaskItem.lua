-- chunkname: @modules/logic/versionactivity2_2/eliminate/view/map/EliminateTaskItem.lua

module("modules.logic.versionactivity2_2.eliminate.view.map.EliminateTaskItem", package.seeall)

local EliminateTaskItem = class("EliminateTaskItem", ListScrollCell)

function EliminateTaskItem:init(go)
	self.viewGO = go
...
function EliminateTaskItem:addEventListeners()
...
function EliminateTaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

EliminateTaskItem.FinishKey = "EliminateTaskItem FinishKey"

function EliminateTaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(EliminateTaskItem.FinishKey)
...
function EliminateTaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function EliminateTaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(EliminateTaskItem.FinishKey)
	...
	if self.taskMo.getAll then
		...
		EliminateRpc.instance:sendGetMatch3WarChessTaskRewardRequest(self.co.id)
	end
end

function EliminateTaskItem:_editableInitView()
	self.rewardItemList = {}
end

function EliminateTaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function EliminateTaskItem:refreshNormalUI()
...
function EliminateTaskItem:refreshRewardItems()
...
function EliminateTaskItem:refreshGetAllUI()
	return
end

function EliminateTaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function EliminateTaskItem:getAnimator()
	return self.animator
end

function EliminateTaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return EliminateTaskItem
