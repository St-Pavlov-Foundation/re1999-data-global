-- chunkname: @modules/logic/versionactivity2_2/dungeon/view/task/VersionActivity2_2TaskItem.lua

module("modules.logic.versionactivity2_2.dungeon.view.task.VersionActivity2_2TaskItem", package.seeall)

local VersionActivity2_2TaskItem = class("VersionActivity2_2TaskItem", ListScrollCell)

function VersionActivity2_2TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity2_2TaskItem:addEventListeners()
...
function VersionActivity2_2TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
end

function VersionActivity2_2TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity2_2TaskView)
		end
	end
end

function VersionActivity2_2TaskItem:_btnFinishAllOnClick()
	self:_btnFinishOnClick()
end

VersionActivity2_2TaskItem.FinishKey = "VersionActivity2_2TaskItem FinishKey"

function VersionActivity2_2TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity2_2TaskItem.FinishKey)
...
function VersionActivity2_2TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity2_2TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity2_2TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity2_2Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity2_2TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity2_2TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity2_2TaskItem:refreshNormalUI()
...
function VersionActivity2_2TaskItem:refreshRewardItems()
...
function VersionActivity2_2TaskItem:refreshGetAllUI()
	return
end

function VersionActivity2_2TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity2_2TaskItem:getAnimator()
	return self.animator
end

function VersionActivity2_2TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity2_2TaskItem
