...
slot0 = class("LoperaTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act168TaskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._act168TaskMO.config.jumpId)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	slot0:_onOneClickClaimReward(slot0._act168TaskMO.activityId)
...
function slot0._btngetallOnClick(slot0)
	LoperaController.instance:dispatchEvent(LoperaEvent.OneClickClaimReward, slot0._act168TaskMO.activityId)
	UIBlockHelper.instance:startBlock("LoperaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._act168TaskMO.config.id)
end
...
function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act168TaskMO and slot0._act168TaskMO.activityId == slot1 and slot0._act168TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	slot0._act168TaskMO = slot1
	slot0._scrollRewards.parentGameObject = slot0._view._csListScroll.gameObject

	slot0:_refreshUI()
	slot0:_moveByRankDiff(Activity168TaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._refreshUI(slot0)
	if not slot0._act168TaskMO then
		return
	end
...
slot0.prefabPath = "ui/viewres/versionactivity_2_2/v2a2_lopera/v2a2_lopera_taskitem.prefab"
