...
slot0 = class("VersionActivity2_2EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function slot0.afterSetData(slot0)
	...
	if not slot0.activityCo then
		...
		logError("VersionActivity2_2EnterViewTabItemBase.afterSetData error, no act config, actId:%s", slot0.actId)

		return
	end

	if not string.nilorempty(string.split(slot0.activityCo.tabBgPath, "#")[1]) then
		UISpriteSetMgr.instance:setV2a2MainActivitySprite(slot0.imageSelectTabIcon, slot2)
	end

	if not string.nilorempty(slot1[2]) then
		UISpriteSetMgr.instance:setV2a2MainActivitySprite(slot0.imageUnSelectTabIcon, slot3)
	end
...
function slot0.refreshNormalTag(slot0)
	...
	if VersionActivity2_2EnterHelper.GetIsShowTabRemainTime(slot0.actId) and slot1 then
		if VersionActivity2_2Enum.MaxShowTimeOffset < slot1:getRealEndTimeStamp() - ServerTime.now() then
			return
		end
