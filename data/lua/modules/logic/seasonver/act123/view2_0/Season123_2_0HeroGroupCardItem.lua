...
slot0 = class("Season123_2_0HeroGroupCardItem", UserDataDispose)
	...
	if slot0.posUnlock then
		if slot0:getEquipId(slot1, slot3) ~= nil and slot4 ~= 0 then
			if not slot0._seasonCardItem then
				slot0._seasonCardItem = Season123_2_0CelebrityCardItem.New()
...
function slot0.canExchange(slot0, slot1)
	...
	if slot14 or slot16 then
		if slot14 then
			GameFacade.showToast(Season123_2_0EquipItem.Toast_Pos_Wrong, slot15)
		end

		if slot16 then
			GameFacade.showToast(Season123_2_0EquipItem.Toast_Pos_Wrong, slot17)
		end
