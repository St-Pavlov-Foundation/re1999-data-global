...
slot0 = class("Season123_2_0TaskMapItem", LuaCompBase)
...
function slot0.refreshUI(slot0)
	...
	if slot3.stageMap[slot0.stage] then
		slot7 = slot6.minRound
		slot0._txttime.text = tostring(slot7)

		gohelper.setActive(slot0._gofinish, slot7 > 0)
	else
		gohelper.setActive(slot0._gofinish, false)
	end

	for slot10 = 1, slot5 do
		gohelper.setActive(slot0.progressItemList[slot10].progressGO, true)
		gohelper.setActive(slot0.progressItemList[slot10].lightIcon, slot10 <= slot4 and slot10 ~= slot5)
		gohelper.setActive(slot0.progressItemList[slot10].darkIcon, slot4 < slot10 and slot4 < slot5)
		gohelper.setActive(slot0.progressItemList[slot10].redIcon, slot10 == slot4 and slot10 == slot5)
	end

	for slot10 = slot5 + 1, #slot0.progressItemList do
		gohelper.setActive(slot0.progressItemList[slot10].progressGO, false)
	end

	slot0._canvasGroup.alpha = Season123TaskModel.instance.curStage == slot0.stage and 1 or 0.5
	slot7 = Season123TaskModel.instance.curStage == slot0.stage and 1 or 0.7

	transformhelper.setLocalScale(slot0._goroot.transform, slot7, slot7, slot7)
end
