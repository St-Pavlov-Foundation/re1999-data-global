...
slot0 = class("Season123EntryView", BaseView)
...
function slot0.checkSceneLoaded(slot0)
	...
	if gohelper.findChild(CameraMgr.instance:getSceneRoot(), "Season123EntryScene/scene") then
		gohelper.setActive(slot0._goentranceitem, true)

		slot0._centerItem = Season123EntryItem.New()
...
function slot0.onCloseView(slot0, slot1)
	if slot1 == ViewName.Season123EpisodeListView then
		slot0._anim:Play(uv0.Anim.Switch1, 0, 0)
	end

	if slot1 == ViewName.Season123RetailView then
		slot0._anim:Play(uv0.Anim.Switch, 0, 0)
	end

	if slot1 == ViewName.Season123StoryPagePopView then
		slot0._anim:Play(uv0.Anim.Switch, 0, 0)
