-- chunkname: @modules/logic/seasonver/act123/view/Season123DecomposeView.lua

module("modules.logic.seasonver.act123.view.Season123DecomposeView", package.seeall)

local Season123DecomposeView = class("Season123DecomposeView", BaseView)

function Season123DecomposeView:onInitView()
...
function Season123DecomposeView:addEvents()
...
function Season123DecomposeView:removeEvents()
...
function Season123DecomposeView:_btnminOnClick()
	self.curCount = self.minDecomposeCount

	self:refreshInfoBySelectCount()
end

function Season123DecomposeView:_btnsubOnClick()
...
function Season123DecomposeView:_btnaddOnClick()
...
function Season123DecomposeView:_btnmaxOnClick()
	self.curCount = self.maxDecomposeCount

	self:refreshInfoBySelectCount()
end

function Season123DecomposeView:_btndecomposeOnClick()
...
function Season123DecomposeView:sendDecomposeEquipRequest()
	local list = {}
...
function Season123DecomposeView:_onEndEdit()
...
function Season123DecomposeView:_editableInitView()
	return
end

function Season123DecomposeView:onOpen()
...
function Season123DecomposeView:refreshUI()
...
function Season123DecomposeView:refreshInfoBySelectCount()
	self._inputvalue:SetText(self.curCount)

	self._txtgetCoin.text = luaLang("multiple") .. self.curCount * self.itemConfig.decomposeGet
end

function Season123DecomposeView:CreateCardIcon()
	if not self.cardIcon then
		local path = self.viewContainer:getSetting().otherRes[1]
		local go = self:getResInst(path, self._gocardpos, "icon")

		self.cardIcon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season123CelebrityCardEquip)
	end
...
function Season123DecomposeView:showMaterialInfoTip()
	MaterialTipController.instance:showMaterialInfo(MaterialEnum.MaterialType.Season123EquipCard, self.itemId)
end

function Season123DecomposeView:onClose()
	return
end

function Season123DecomposeView:onDestroyView()
	if self.cardIcon then
		self.cardIcon:disposeUI()
	end
end

return Season123DecomposeView
