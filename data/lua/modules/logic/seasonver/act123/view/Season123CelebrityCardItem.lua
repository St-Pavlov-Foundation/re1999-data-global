-- chunkname: @modules/logic/seasonver/act123/view/Season123CelebrityCardItem.lua

module("modules.logic.seasonver.act123.view.Season123CelebrityCardItem", package.seeall)

local Season123CelebrityCardItem = class("Season123CelebrityCardItem", LuaCompBase)

Season123CelebrityCardItem.AssetPath = "ui/viewres/seasonver/act123/season123celebritycarditem.prefab"

function Season123CelebrityCardItem:init(go, equipId, params)
	self.go = go
	self._equipId = equipId

	local assetPath = Season123CelebrityCardItem.AssetPath
...
function Season123CelebrityCardItem:handleCardLoaded()
	self._cardGo = self._resLoader:getInstGO()
	self._icon = MonoHelper.addNoUpdateLuaComOnceToGo(self._cardGo, Season123CelebrityCardEquip)
...
function Season123CelebrityCardItem:showTag(enable)
...
function Season123CelebrityCardItem:showProbability(enable)
	self._showTag = enable
...
function Season123CelebrityCardItem:showNewFlag(enable)
...
function Season123CelebrityCardItem:showNewFlag2(enable)
...
function Season123CelebrityCardItem:reset(equipId)
	self._equipId = equipId
...
function Season123CelebrityCardItem:setColorDark(enable)
...
function Season123CelebrityCardItem:destroy()
	if self._icon then
...
return Season123CelebrityCardItem
