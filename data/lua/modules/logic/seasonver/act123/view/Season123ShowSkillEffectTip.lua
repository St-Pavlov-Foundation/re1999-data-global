-- chunkname: @modules/logic/seasonver/act123/view/Season123ShowSkillEffectTip.lua

module("modules.logic.seasonver.act123.view.Season123ShowSkillEffectTip", package.seeall)

local Season123ShowSkillEffectTip = class("Season123ShowSkillEffectTip", LuaCompBase)

function Season123ShowSkillEffectTip:init(go)
...
function Season123ShowSkillEffectTip:_btnclosebuffOnClick()
	gohelper.setActive(self._goBuffContainer, false)
...
function Season123ShowSkillEffectTip:addLink(desc)
	if string.nilorempty(desc) then
		return ""
	end

	desc = string.gsub(desc, "%[(.-)%]", Season123ShowSkillEffectTip.addLinkTag1)
	desc = string.gsub(desc, "%【(.-)%】", Season123ShowSkillEffectTip.addLinkTag2)

	return desc
end

function Season123ShowSkillEffectTip.addLinkTag1(skillEffectName)
...
function Season123ShowSkillEffectTip.addLinkTag2(skillEffectName)
	skillEffectName = SkillConfig.instance:processSkillDesKeyWords(skillEffectName)
...
function Season123ShowSkillEffectTip:addHyperLinkClick(txtDesc)
...
function Season123ShowSkillEffectTip:onHyperLinkClick(id, clickPosition)
...
function Season123ShowSkillEffectTip:createAndGetBufferItem(effectId)
...
function Season123ShowSkillEffectTip:addChildHyperLinkClick(txtDesc)
	local hyperLinkClick = gohelper.onceAddComponent(txtDesc, typeof(ZProj.TMPHyperLinkClick))

	hyperLinkClick:SetClickListener(self.onChildHyperLinkClick, self)
end

function Season123ShowSkillEffectTip:onChildHyperLinkClick(id, clickPosition)
	local effectId = tonumber(id)

	self:createAndGetBufferItem(effectId)
end

function Season123ShowSkillEffectTip:onDestroy()
	self._btnclosebuff:RemoveClickListener()
end

return Season123ShowSkillEffectTip
