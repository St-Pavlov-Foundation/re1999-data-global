...
slot0 = class("Season123ShowSkillEffectTip", LuaCompBase)
...
function slot0.addLink(slot0, slot1)
	...
	return string.gsub(string.gsub(slot1, "%[(.-)%]", uv0.addLinkTag1), "%【(.-)%】", uv0.addLinkTag2)
end

function slot0.addLinkTag1(slot0)
	if not SkillConfig.instance:getSkillEffectDescCoByName(SkillConfig.instance:processSkillDesKeyWords(slot0)) or slot1.isNoShow4ActiveSkill == 1 or slot1.isSpecialCharacter == 1 then
...
function slot0.addLinkTag2(slot0)
	if not SkillConfig.instance:getSkillEffectDescCoByName(SkillConfig.instance:processSkillDesKeyWords(slot0)) or slot1.isNoShow4ActiveSkill == 1 or slot1.isSpecialCharacter == 1 then
		return string.format("【%s】", slot0)
	end
...
function slot0.createAndGetBufferItem(slot0, slot1)
	...
	slot2.txtBuffDesc.text = slot0:addLink(HeroSkillModel.instance:skillDesToSpot(slot2.config.desc))
