...
slot0 = class("Season123HeroGroupListView", BaseView)
...
function slot0._editableInitView(slot0)
	...
	for slot4 = 1, 4 do
		...
		table.insert(slot0._heroItemList, MonoHelper.addNoUpdateLuaComOnceToGo(slot7, Season123HeroGroupHeroItem, slot0))
	...
	for slot4 = 1, 4 do
		table.insert(slot0._bgList, gohelper.findChild(slot0.viewGO, "herogroupcontain/hero/bg" .. slot4 .. "/bg"))
	end
...
function slot0._onEndDrag(slot0, slot1, slot2)
	...
	if not slot11 or not slot13 then
		...
		if not slot11 then
			GameFacade.showToast(Season123EquipItem.Toast_Pos_Wrong, slot12)
		end

		if not slot13 then
			GameFacade.showToast(Season123EquipItem.Toast_Pos_Wrong, slot14)
		end
...
function slot0._updateHeroList(slot0)
	for slot4, slot5 in ipairs(slot0._heroItemList) do
		if Season123HeroGroupModel.instance:isEpisodeSeason123() or Season123HeroGroupModel.instance:isEpisodeSeason123Retail() then
			...
			slot7 = {}
			slot7 = HeroSingleGroupMO.New()
			slot7.id = slot4
			slot7.heroUid = slot6.heroList[slot4]

			if slot6.isReplay then
				slot5:onUpdateMO(slot7, slot0.viewParam, slot6.replay_hero_data[slot7.heroUid])
			else
				slot5:onUpdateMO(slot7, slot0.viewParam)
			end
		else
