-- chunkname: @modules/logic/seasonver/act123/view/Season123PickHeroEntryItem.lua

module("modules.logic.seasonver.act123.view.Season123PickHeroEntryItem", package.seeall)

local Season123PickHeroEntryItem = class("Season123PickHeroEntryItem", UserDataDispose)

function Season123PickHeroEntryItem:ctor()
	self:__onInit()
end

function Season123PickHeroEntryItem:dispose()
	self:removeEvents()
	self:__onDispose()
end

function Season123PickHeroEntryItem:init(viewGO)
	self.viewGO = gohelper.findChild(viewGO, "root")

	self:initComponent()
end

Season123PickHeroEntryItem.exSkillFillAmount = {
	0.2,
	0.4,
	0.6,
	0.79,
...
Season123PickHeroEntryItem.MaxRare = 5

function Season123PickHeroEntryItem:initComponent()
	...
	local maxRank = HeroConfig.instance:getMaxRank(Season123PickHeroEntryItem.MaxRare)
...
function Season123PickHeroEntryItem:removeEvents()
...
function Season123PickHeroEntryItem:initData(index)
	self._index = index
...
function Season123PickHeroEntryItem:refreshUI()
		...
		if not isEmpty then
			...
			if mo.heroMO then
				...
				if not skinConfig then
					logError("Season123PickHeroEntryItem.refreshUI error, skinCfg is nil, id:" .. tostring(mo.skinId))
...
function Season123PickHeroEntryItem:refreshRare(heroMO)
	for rare = 1, #self._rareList do
		gohelper.setActive(self._rareList[rare], rare <= heroMO.config.rare + 1)
	end
end

function Season123PickHeroEntryItem:refreshRank(heroMO)
...
function Season123PickHeroEntryItem:refreshExSkill(heroMO)
...
function Season123PickHeroEntryItem:onClickSelf()
...
function Season123PickHeroEntryItem:onClickSupport()
	if self._index == Activity123Enum.SupportPosIndex then
		local mo = Season123PickHeroEntryModel.instance:getByIndex(self._index)
...
function Season123PickHeroEntryItem:OpenAnim()
...
function Season123PickHeroEntryItem:playOpenAnim()
	gohelper.setActive(self.viewGO, true)
end

function Season123PickHeroEntryItem:cutHeroAnim(isCut)
	gohelper.setActive(self._gocuthero, isCut)
	self._animCuthero:Play("pickheroentryview_click", 0, 0)
end

return Season123PickHeroEntryItem
