-- chunkname: @modules/logic/seasonver/act123/view/Season123StoryCoverItem.lua

module("modules.logic.seasonver.act123.view.Season123StoryCoverItem", package.seeall)

local Season123StoryCoverItem = class("Season123StoryCoverItem", LuaCompBase)

function Season123StoryCoverItem:ctor(param)
	self.param = param
end

function Season123StoryCoverItem:init(go)
	self:__onInit()
...
function Season123StoryCoverItem:addEventListeners()
	self.btnClick:AddClickListener(self.onClickCoverItem, self)
end

function Season123StoryCoverItem:removeEventListeners()
	self.btnClick:RemoveClickListener()
end

Season123StoryCoverItem.unlockDelayTime = 1.16

function Season123StoryCoverItem:refreshItem()
...
function Season123StoryCoverItem:onClickCoverItem()
...
function Season123StoryCoverItem:refreshUnlockState(unlockState)
	if self.isUnlock and self.isUnlock ~= unlockState then
		...
		TaskDispatcher.runDelay(self.playUnlockAnim, self, Season123StoryCoverItem.unlockDelayTime)
...
function Season123StoryCoverItem:playUnlockAnim()
...
function Season123StoryCoverItem:onUnlockAnimDone()
	gohelper.setActive(self.goLocked, not self.isUnlock)
	UIBlockMgr.instance:endBlock("playCoverItemUnlockAnim")
end

function Season123StoryCoverItem:destroy()
	self:__onDispose()
	UIBlockMgr.instance:endBlock("playCoverItemUnlockAnim")
	TaskDispatcher.cancelTask(self.playUnlockAnim, self)
end

return Season123StoryCoverItem
