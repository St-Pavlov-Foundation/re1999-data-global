...
function Season123_1_8EquipView:_rebuildLayout_overseas(listItem)
	...
	self._txtDescFrameTimer = FrameTimerController.instance:register(function()
		for _, item in pairs(listItem) do
			local text = item.txtDesc
			local trans = text.transform
			local v2 = text:GetPreferredValues()

			recthelper.setHeight(trans, v2.y)
		end
	end, nil, 1, 1)
...
function Season123_1_8EquipView:_editableInitView()
	...
	self._slotItems = {}
	self._descItems = {}
	self._skillContainer = MonoHelper.addNoUpdateLuaComOnceToGo(self._gobuffContainer, Season123_1_8ShowSkillEffectTip)
end
...
function Season123_1_8EquipView:refreshSkills(itemCfg, descItem, colorStr)
	...
	self:_rebuildLayout_overseas(descItem.skillItems)
end
