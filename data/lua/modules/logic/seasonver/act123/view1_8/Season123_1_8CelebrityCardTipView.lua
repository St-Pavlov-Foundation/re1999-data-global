...
slot0 = class("Season123_1_8CelebrityCardTipView", BaseView)
...
function slot0.onOpen(slot0)
	...
	slot0._activityId = slot0.viewParam.actId or Season123Model.instance:getCurSeasonId()
...
function slot0.refreshQuantity(slot0)
	if slot0.viewParam.needQuantity then
		...
		slot1 = nil
		slot0._txthadnumber.text = formatLuaLang("materialtipview_itemquantity", (not slot0.viewParam.fakeQuantity or tostring(slot0.viewParam.fakeQuantity)) and (not slot0._activityId or tostring(GameUtil.numberDisplay(Season123EquipMetaUtils.getEquipCount(slot0._activityId, slot0._itemId)))) and tostring(Season123EquipMetaUtils.getCurSeasonEquipCount(slot0._itemId)))
	else
...
function slot0.refreshProps(slot0)
	slot1 = false
	slot3 = Season123EquipMetaUtils.getCareerColorBrightBg(slot0._itemId)
	slot4 = {
		[slot10] = true
	}

	for slot8, slot9 in ipairs(Season123EquipMetaUtils.getEquipPropsStrList(slot0._itemCfg.attrId, true)) do
...
function slot0.refreshSkills(slot0)
	slot2 = Season123EquipMetaUtils.getCareerColorBrightBg(slot0._itemId)
	slot3 = false
	slot4 = {
		[slot10] = true
	}

	for slot8, slot9 in ipairs(Season123EquipMetaUtils.getSkillEffectStrList(slot0._itemCfg)) do
...
function slot0.checkCreateIcon(slot0)
	if not slot0._icon then
		slot0._icon = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], slot0._gocard, "icon"), Season123_1_8CelebrityCardEquip)
	end
end
