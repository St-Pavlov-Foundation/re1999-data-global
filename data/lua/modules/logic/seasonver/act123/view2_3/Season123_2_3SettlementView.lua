...
slot0 = class("Season123_2_3SettlementView", BaseViewExtended)
...
function slot0._showGetCardView(slot0)
	if slot0.equip_cards then
		...
		slot0._showTipsFlow:addWork(Season123_2_3SettlementTipsWork.New())
...
function slot0._onRewardItemShow(slot0, slot1, slot2, slot3)
	...
	if slot2.materilType == MaterialEnum.MaterialType.Season123EquipCard then
		Season123_2_3CelebrityCardItem.New():init(gohelper.findChild(gohelper.findChild(slot1, "go_prop"), "cardicon"), slot2.materilId)
		...
		for slot15 = 1, 4 do
			if slot8[slot15] then
				if lua_hero_trial.configDict[slot8[slot15].trialId][0] then
					table.insert(slot0._heroList, slot0:openSubView(Season123_2_3SettlementHeroItem, slot0._hero_obj_list[slot15], nil, slot7, nil, slot8[slot15].equipUid, slot11[tostring(slot16.heroId - 1099511627776.0)] and slot11[slot17].equipUid, nil, slot8[slot15]))
				end
			elseif slot16 ~= "0" then
				table.insert(slot0._heroList, slot0:openSubView(Season123_2_3SettlementHeroItem, slot0._hero_obj_list[slot15], nil, slot7, slot16, slot9[slot15] ~= "0" and slot10[slot16] and slot10[slot16].equipUid, slot16 ~= "0" and slot11[slot16] and slot11[slot16].equipUid))
			end
		end
...
function slot0.setMainPosCardItemByUid(slot0, slot1, slot2)
	if not Season123Model.instance:getActInfo(slot0._battleContext.actId).unlockIndexSet[Season123Model.instance:getUnlockCardIndex(Activity123Enum.MainCharPos, slot1)] then
		...
		if slot2 and slot2 ~= "0" then
			...
			slot0:openSubView(Season123_2_3CelebrityCardGetItem, Season123_2_3CelebrityCardItem.AssetPath, slot8, slot2, true)
		else
...
function slot0._onGetFightRecordGroupReply(slot0, slot1)
	for slot5 = 1, 4 do
		...
		if slot7 ~= "0" then
			table.insert(slot0._heroList, slot0:openSubView(Season123_2_3SettlementHeroItem, slot6, nil, true, slot7, slot8, slot9, slot10))
		end
	end
...
function slot0.setMainPosCardItemById(slot0, slot1, slot2)
	...
	if not Season123Model.instance:getActInfo(slot0._battleContext.actId).unlockIndexSet[Season123Model.instance:getUnlockCardIndex(Activity123Enum.MainCharPos, slot1)] then
		...
		if slot3 and slot3 ~= 0 then
			...
			slot0:openSubView(Season123_2_3CelebrityCardGetItem, Season123_2_3CelebrityCardItem.AssetPath, slot9, nil, , slot3)
		else
			gohelper.setActive(slot8, true)
