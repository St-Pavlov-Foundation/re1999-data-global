...
slot0 = class("Season123_2_3EpisodeLoadingView", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0._btnskipAnim = gohelper.findChildButtonWithAudio(slot0.viewGO, "#btn_skipAnim")
...
function slot0.addEvents(slot0)
	slot0._btnskipAnim:AddClickListener(slot0._btnSkilAnimOnClick, slot0)
end

function slot0.removeEvents(slot0)
	slot0._btnskipAnim:RemoveClickListener()
end

function slot0._btnSkilAnimOnClick(slot0)
	gohelper.setActive(slot0._btnskipAnim.gameObject, false)
	slot0._viewAnim:Play("season123episodeloadingview_open", 0, 0.7)
	TaskDispatcher.cancelTask(slot0.handleDelayAnimTransition, slot0)
	TaskDispatcher.runDelay(slot0.handleDelayAnimTransition, slot0, 0.2)

	if slot0.audioId then
		AudioMgr.instance:stopPlayingID(slot0.audioId)
...
function slot0.onOpen(slot0)
	...
	logNormal(string.format("Season123_2_3EpisodeLoadingView actId=%s, stage=%s", slot1, slot2))
	Season123EpisodeLoadingController.instance:onOpenView(slot1, slot2, slot0.viewParam.layer)

	slot0.audioId = AudioMgr.instance:trigger(AudioEnum.Season123.play_ui_jinye_film_slide)
