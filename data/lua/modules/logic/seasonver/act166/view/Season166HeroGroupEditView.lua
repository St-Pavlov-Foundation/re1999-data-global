...
slot0 = class("Season166HeroGroupEditView", BaseView)
...
function slot0._btnconfirmOnClick(slot0)
	...
	slot1 = Season166HeroSingleGroupModel.instance:getById(slot0._singleGroupMOId)
	...
	if slot0._heroMO then
		...
		slot2, slot3 = Season166HeroSingleGroupModel.instance:hasHeroUids(slot0._heroMO.uid, slot0._singleGroupMOId)

		if slot2 then
			Season166HeroSingleGroupModel.instance:removeFrom(slot3)
			Season166HeroSingleGroupModel.instance:addTo(slot0._heroMO.uid, slot0._singleGroupMOId)
		...
		if Season166HeroSingleGroupModel.instance:isAidConflict(slot0._heroMO.heroId) then
		...
		if Season166HeroSingleGroupModel.instance.assistMO and slot1.heroUid == slot2.heroUid then
			Season166HeroGroupModel.instance:cleanAssistData()
			HeroGroupController.instance:dispatchEvent(HeroGroupEvent.OnModifyHeroGroup)
		end

		Season166HeroSingleGroupModel.instance:removeFrom(slot0._singleGroupMOId)
		slot0:_saveCurGroupInfo()
		slot0:closeThis()
	end
end
...
function slot0._btncharacterOnClick(slot0)
	if slot0._heroMO then
		slot1 = {}
		slot1 = (not slot0._isShowQuickEdit or Season166HeroGroupQuickEditModel.instance:getList()) and Season166HeroGroupEditModel.instance:getList()

		if slot0._heroMO:isOtherPlayerHero() and Season166HeroGroupEditModel.instance:getEquipMOByHeroUid(slot0._heroMO.uid) then
...
function slot0._btnquickeditOnClick(slot0)
	...
	if slot0._isShowQuickEdit then
		...
		if Season166HeroGroupQuickEditModel.instance:getById(slot0._originalHeroUid) then
			Season166HeroGroupQuickEditModel.instance:selectCell(Season166HeroGroupQuickEditModel.instance:getIndex(slot1), true)
		end
	else
		slot0:_saveQuickGroupInfo()
		slot0:_onHeroItemClick(nil)
		...
		if Season166HeroSingleGroupModel.instance:getHeroUid(slot0._singleGroupMOId) ~= "0" then
			Season166HeroGroupEditModel.instance:selectCell(Season166HeroGroupEditModel.instance:getIndex(Season166HeroGroupEditModel.instance:getById(slot1)), true)
		end
...
function slot0.replaceSelectHeroDefaultEquip(slot0)
	if slot0._heroMO and slot0._heroMO:hasDefaultEquip() then
		for slot6, slot7 in pairs(Season166HeroGroupModel.instance:getCurGroupMO().equips) do
...
function slot0.replaceHeroesDefaultEquip(slot0, slot1)
	slot3 = Season166HeroGroupModel.instance:getCurGroupMO().equips
	slot4 = nil
...
function slot0._saveQuickGroupInfo(slot0)
	if Season166HeroGroupQuickEditModel.instance:getIsDirty() then
		slot2 = Season166HeroGroupModel.instance:getCurGroupMO()
			...
			if slot1[slot6] ~= nil then
				...
				if tonumber(slot7) < 0 then
					if HeroGroupTrialModel.instance:getById(slot7) then
						Season166HeroSingleGroupModel.instance:getByIndex(slot6):setTrial(slot9.trialCo.id, slot9.trialCo.trialTemplate)
					else
						slot8:setTrial()
					end
				else
...
function slot0._onGroupModify(slot0)
	if slot0._isShowQuickEdit then
		...
		if slot0._originalHeroUid ~= Season166HeroSingleGroupModel.instance:getHeroUid(slot0._singleGroupMOId) then
			slot0._originalHeroUid = slot1

			Season166HeroGroupEditModel.instance:setParam(slot1)
			slot0:_onHeroItemClick(nil)
			Season166HeroGroupEditModel.instance:cancelAllSelected()
			Season166HeroGroupEditModel.instance:selectCell(Season166HeroGroupEditModel.instance:getIndex(Season166HeroGroupEditModel.instance:getById(slot1)), true)
		end
...
function slot0.onOpen(slot0)
	...
	Season166HeroGroupEditModel.instance:setParam(slot0._originalHeroUid)

	slot0._heroMO = Season166HeroGroupEditModel.instance:copyCharacterCardList(true)
