...
slot0 = class("Season166HeroSingleGroupModel", ListScrollModel)
...
function slot0._buildMOList(slot0)
	...
	for slot5 = 1, ModuleEnum.MaxHeroCountInGroup do
		table.insert(slot1, Season166HeroSingleGroupMO.New())
	end
...
function slot0.setMaxHeroCount(slot0, slot1)
	...
	for slot6 = 1, slot1 do
		table.insert(slot2, Season166HeroSingleGroupMO.New())
	end
...
function slot0.setSingleGroup(slot0, slot1, slot2)
	...
	if slot2 and slot1 then
		for slot8 = 1, #slot0:getList() do
			...
			elseif slot0.assistMO and slot0.assistMO.heroUid == slot4[slot8].heroUid then
				slot4[slot8] = slot0.assistMO
			else
...
function slot0.setAssistHeroGroupMO(slot0, slot1)
	slot0.assistMO = slot1

	if slot0.assistMO then
		slot0:getCurGroupMO().heroList[slot1.id] = slot1.heroUid
	end
end

function slot0.checkIsMainHero(slot0, slot1)
	slot3 = 0

	for slot7, slot8 in ipairs(slot0:getList()) do
		if slot8.heroUid == slot1 then
			slot3 = slot7
	...
	return slot3 <= Season166HeroGroupModel.instance:getMaxHeroCountInGroup() / 2, slot3
end

function slot0.isAssistHeroInTeam(slot0)
	if not slot0.assistMO then
		return false
	else
		for slot5, slot6 in ipairs(slot0:getList()) do
			if slot6.heroUid == slot0.assistMO.heroUid then
				return true
			end
		end
	end
