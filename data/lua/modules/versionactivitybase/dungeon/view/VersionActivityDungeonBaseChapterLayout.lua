...
function slot0.refreshEpisodeNodes(slot0)
	...
	for slot8, slot9 in ipairs(slot1) do
		if slot9 and DungeonModel.instance:getEpisodeInfo(slot9.id) or nil then
			...
			if slot4 then
				recthelper.setAnchorX(slot4.containerTr, (recthelper.getAnchorX(slot10.containerTr) or 0) + slot10.episodeItem:getMaxWidth() + slot0._constDungeonNormalDeltaX)
			else
				recthelper.setAnchorX(slot4.containerTr, slot0._constDungeonNormalPosX)
			end
		end
	end
