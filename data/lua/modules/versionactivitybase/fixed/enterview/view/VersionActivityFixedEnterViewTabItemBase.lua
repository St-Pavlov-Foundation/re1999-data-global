...
local var_0_0 = class("VersionActivityFixedEnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function var_0_0.afterSetData(arg_3_0)
	...
	if not arg_3_0.activityCo then
		...
		logError("VersionActivityFixedEnterViewTabItemBase.afterSetData error, no act config, actId:%s", arg_3_0.actId)
	...
	if not string.nilorempty(var_3_1) then
		VersionActivityFixedHelper.setMainActivitySprite(arg_3_0.imageSelectTabIcon, var_3_1)
	end

	local var_3_2 = var_3_0[2]

	if not string.nilorempty(var_3_2) then
		VersionActivityFixedHelper.setMainActivitySprite(arg_3_0.imageUnSelectTabIcon, var_3_2)
	end
...
function var_0_0.refreshNormalTag(arg_8_0)
	...
	if VersionActivityFixedEnterHelper.GetIsShowTabRemainTime(arg_8_0.actId) and var_8_0 then
		if var_8_0:getRealEndTimeStamp() - ServerTime.now() > VersionActivityFixedHelper.getVersionActivityEnum().MaxShowTimeOffset then
			return
		end
